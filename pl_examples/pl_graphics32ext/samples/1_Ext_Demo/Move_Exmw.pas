...
begin
  ...
  Src.Bitmap.Clear(clGreen32);


  flay:=TExtBitmapLayer.Create(Src.Layers);

  flay.Bitmap.DrawMode:=dmcustom;

  flay.Bitmap.LoadFromFile(pathMedia+'icon1.png');

  //flay.Bitmap.Clear(XClearColor);

  //=====================================================
  frub:=TExtRubberBandLayer.Create(Src.Layers);
  frub.Options:=DefaultRubberbandOptions;
 // frub.HandleSize:=9;
