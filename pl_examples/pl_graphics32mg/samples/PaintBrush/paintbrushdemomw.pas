...
//Ezt csak a prevjϊra hasznαljuk!!, mert a vιgso intensityt majd nem itt, hanem a layer intensitynιl szabαlyozzuk!!!
...
begin
  ...
  if intensity=0 then
  begin
    //elmιletileg elozoleg clearezve lett a kιp, ϊgyhogy nem kιne csinαlunk semmit se, de azιrt νgy tuti!
    for y := LY to RY do
    begin
...
//          P[x]:=SetAlpha(P[x], b);
          ...
          begin
            //P[x] := 0;
            P[x] := color32(255,255,255,255);   //works ok!           kόlso fehιr
//          P[x]:=SetAlpha(P[x], 0);
          end;
        end;
      end;
