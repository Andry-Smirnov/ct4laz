...
type
  ...
  private
    fServer: TswsWebSocketServer;

    procedure OnAfterAddConnection(Server: TswsCustomServer; aConnection: TswsCustomConnection);
    procedure OnBeforeAddConnection(Server: TswsCustomServer; aConnection: TswsCustomConnection; var CanAdd: boolean);
    procedure OnAfterRemoveConnection(Server: TswsCustomServer; aConnection: TswsCustomConnection);
    procedure OnBeforeRemoveConnection(Server: TswsCustomServer; aConnection: TswsCustomConnection);
    procedure OnServerSocketError(Server: TswsCustomServer; Socket: TTCPBlockSocket);

    procedure OnOpen(aSender: TswsWebSockeCustomConnection);
    procedure OnRead(aSender: TswsWebSockeCustomConnection; aFinal, aRes1, aRes2, aRes3: boolean; aCode: integer; aData: TMemoryStream);
    procedure OnWrite(aSender: TswsWebSockeCustomConnection; aFinal, aRes1, aRes2, aRes3: boolean; aCode: integer; aData: TMemoryStream);
    procedure OnClose(aSender: TswsWebSockeCustomConnection; aCloseCode: integer; aCloseReason: string; aClosedByPeer: boolean);
  ...
  TTesTswsWebSocketServerConnection = class;

  { TTesTswsWebSocketServer }

  TTesTswsWebSocketServer = class(TswsWebSocketServer)
  public
    ...
    ): TswsWebSocketServerConnections; override;
  end;

  TTesTswsWebSocketServerConnection = class(TswsWebSocketServerConnection)
  public
...
begin
  ...
  fServer := TTesTswsWebSocketServer.Create(HostCombo.text, PortCombo.Text);
...
begin
  ...
  begin
    ...
    begin
      TswsWebSockeCustomConnection(ConnectionList.Items[i]. Data).SendText(CharsetConversion(SendSelectedMemo.Text, GetCurCP, UTF_8));
...
procedure TMainForm.OnAfterAddConnection(Server: TswsCustomServer;
  aConnection: TswsCustomConnection);
begin
  ...
  begin
    ...
    TswsWebSocketServerConnection(aConnection).OnWrite := OnWrite;
    TswsWebSocketServerConnection(aConnection).OnRead := OnRead;
    TswsWebSocketServerConnection(aConnection).OnClose := OnClose;
    TswsWebSocketServerConnection(aConnection).OnOpen := OnOpen;
    //TswsWebSocketServerConnection(aConnection).Socket.OnSyncStatus := OnConnectionSocket;

  end;
end;

procedure TMainForm.OnBeforeAddConnection(Server: TswsCustomServer;
  aConnection: TswsCustomConnection; var CanAdd: boolean);
begin
  InfoMemo.Lines.Insert(0, Format('OnBeforeAddConnection %d', [aConnection.Index]));
end;

procedure TMainForm.OnAfterRemoveConnection(Server: TswsCustomServer;
  aConnection: TswsCustomConnection);
begin
  InfoMemo.Lines.Insert(0, Format('OnAfterRemoveConnection %d', [aConnection.Index]));
end;

procedure TMainForm.OnBeforeRemoveConnection(Server: TswsCustomServer;
  aConnection: TswsCustomConnection);
var i: integer;
begin
  ...
  for i := 0 to ConnectionList.Items.Count - 1 do
  begin
    if (TswsCustomConnection(ConnectionList.Items[i].Data).Index = aConnection.Index) then
    begin
...
procedure TMainForm.OnServerSocketError(Server: TswsCustomServer; Socket: TTCPBlockSocket);
begin
...
procedure TMainForm.OnOpen(aSender: TswsWebSockeCustomConnection);
begin
  InfoMemo.Lines.Insert(0, Format('OnOpen %d', [aSender.Index]));
end;

procedure TMainForm.OnRead(aSender: TswsWebSockeCustomConnection; aFinal, aRes1,
...
procedure TMainForm.OnWrite(aSender: TswsWebSockeCustomConnection; aFinal, aRes1,
  aRes2, aRes3: boolean; aCode: integer; aData: TMemoryStream);
var item: TListItem;
begin
...
procedure TMainForm.OnClose(aSender: TswsWebSockeCustomConnection;
...
begin
  InfoMemo.Lines.Insert(0, Format('OnConnectionSocket %d, %s, %s', [
    TswsTCPCustomConnectionSocket(Sender).Connection.Index,
...
begin
  ...
  for i := 0 to ConnectionList.Items.Count - 1 do
  begin
    if (TswsCustomConnection(ConnectionList.Items[i].Data).Index = aConnectionIndex) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ConnectionList.Items[i].Selected := false;
      TswsWebSockeCustomConnection(ConnectionList.Items[i]. Data).Close(wsCloseNormal, 'closing connection');
    end;
  end;
  fServer.UnLockTermination;
end;
...
procedure TMainForm.ConnectionListChange(Sender: TObject; Item: TListItem;
  Change: TItemChange);
var s: string;
    c: TTesTswsWebSocketServerConnection;
begin
  ...
  begin
    //TswsWebSockeCustomConnection(ConnectionList.Items[i]. Data).SendText(SendSelectedMemo.Text)
    c := TTesTswsWebSocketServerConnection(Item.data);
...
var ms1, ms2, ms3: TMemoryStream;
    fs: TFileStream;
    l, i: integer;
    c: TswsWebSockeCustomConnection;
begin
  ...
  begin
    ...
    try
      ...
      begin
        if (ConnectionList.Items[i].Selected) then
        begin
          c := TswsWebSockeCustomConnection(ConnectionList.Items[i]. Data);
...
begin
  ...
  begin
    if (ConnectionList.Items[i].Selected) then
    begin
      TswsWebSockeCustomConnection(ConnectionList.Items[i]. Data).Ping('ping you');
    end;
  end;
  fServer.UnLockTermination;
end;
...
begin
  ...
  begin
    if (ConnectionList.Items[i].Selected) then
    begin
      TswsWebSockeCustomConnection(ConnectionList.Items[i]. Data).Pong('pong you');
    end;
  end;
  fServer.UnLockTermination;
end;
...
function TTesTswsWebSocketServer.GetWebSocketConnectionClass(
      Socket: TswsTCPCustomConnectionSocket;
      Header: TStringList;
      ResourceName, Host, Port, Origin, Cookie: string;
      out HttpResult: integer;
      var Protocol, Extensions: string
    ): TswsWebSocketServerConnections;
begin
  Result:= TTesTswsWebSocketServerConnection;
end;

initialization
  {$I websocketservermw.lrs}
