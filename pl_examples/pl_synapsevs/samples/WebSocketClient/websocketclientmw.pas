...
type
  ...
  { TTesTswsWebSocketClientConnection }

  TTesTswsWebSocketClientConnection = class(TswsWebSocketClientConnection)
    fFramedText: string;
  ...
  private
    fClient: TswsWebSocketClientConnection;
    fFrameString: string;
...
begin
  fClient := TTesTswsWebSocketClientConnection.Create(HostCombo.text, PortCombo.Text, '/');
...
var s: string;
    c: TTesTswsWebSocketClientConnection;
begin
  c := TTesTswsWebSocketClientConnection(aSender);
...
procedure TTesTswsWebSocketClientConnection.ProcessText(aFinal, aRes1, aRes2,
  aRes3: boolean; aData: string);
begin
  fFramedText := aData;
end;

procedure TTesTswsWebSocketClientConnection.ProcessTextContinuation(aFinal, aRes1,
...
procedure TTesTswsWebSocketClientConnection.ProcessStream(aFinal, aRes1, aRes2,
...
procedure TTesTswsWebSocketClientConnection.ProcessStreamContinuation(aFinal,
...
procedure TTesTswsWebSocketClientConnection.ProcessPing(aData: string);
begin
  Pong(aData);
  fPing := aData;
...
procedure TTesTswsWebSocketClientConnection.ProcessPong(aData: string);
begin
  fPong := aData;
  Synchronize(SyncPong);
end;

procedure TTesTswsWebSocketClientConnection.SyncTextFrame;
begin
  MainForm.FrameReceiveMemo.Text :=  CharsetConversion(fFramedText, UTF_8, GetCurCP);
end;

procedure TTesTswsWebSocketClientConnection.SyncBinFrame;
...
procedure TTesTswsWebSocketClientConnection.SyncPing;
begin
  MainForm.InfoMemo.Lines.Insert(0, Format('SyncPing %s', [fPing]));
end;

procedure TTesTswsWebSocketClientConnection.SyncPong;
begin
  MainForm.InfoMemo.Lines.Insert(0, Format('SyncPong %s', [fPong]));
end;

constructor TTesTswsWebSocketClientConnection.Create(aHost, aPort, aResourceName: string;
    aOrigin: string = '-'; aProtocol: string = '-'; aExtension: string = '-';
...
destructor TTesTswsWebSocketClientConnection.Destroy;
begin
  fFramedStream.free;
  inherited;
end;
