...
begin
  ...
  //    WriteLn('ERROR : TvkObj_ConsoleDebugReporter.Create');
    ...
    WriteLn('  done');
     ...
     //==>> 5555 ========================================
    //////////////////////////////////////////////////////////////////////////////
    WriteLn('create depth stencil image and image view');
    ImageFactory     := TvkObj_ImageFactory.Create(fDevice);
    ImageViewFactory := TvkObj_ImageViewFactory.Create(fDevice);
    try
      ImageFactory.ImageType    := VK_IMAGE_TYPE_2D;
      ImageFactory.Format       := fDepthFormat;
      ImageFactory.Extent       := vkoExtent3D(
        SurfaceCapabilities.currentExtent.width,
        SurfaceCapabilities.currentExtent.height,
        1);
      ImageFactory.MipLevels    := 1;
     ...
     //-------------- SOS -----------
     fDepthStencilImage.AllocateAndBindImageMemory(fPhyDevice.GetMemoryProperties,@AllocCallbacks);
      ...
      fDepthStencilImageView := ImageViewFactory.CreateImageView;
      WriteLn('  view handle: ', PtrUInt(fDepthStencilImageView.Handle));

      WriteLn('  done');
    finally
      FreeAndNil(ImageViewFactory);
      FreeAndNil(ImageFactory);
    end;
    ...
    try
      ...
      FrameBufferFactory.Attachments[1]     := fDepthStencilImageView.Handle;  //==>> 5555
