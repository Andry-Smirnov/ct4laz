...
const
  Graphics32Version    = '7.3.1';
  Graphics32VersionNum = 731;
...
{$IFDEF FPC}
  Clipbrd,
  {$IFDEF LCLWin32}     // ct9999 for CodeTyphon Studio
    GR32_Backends_LCL_Win,
  {$ENDIF}
  {$IFDEF LCLWince}     // ct9999 for CodeTyphon Studio
    GR32_Backends_LCL_Wince,
  {$ENDIF}
  {$IFDEF LCLQT}        // ct9999 for CodeTyphon Studio
    GR32_Backends_LCL_Qt,
  {$IFEND}
  {$IFDEF LCLQT5}        // ct9999 for CodeTyphon Studio
    GR32_Backends_LCL_Qt5,
  {$IFEND}
  {$IFDEF LCLfpgui}        // ct9999 for CodeTyphon Studio
    GR32_Backends_LCL_FpGUI,
  {$IFEND}
...
//=== ct9999 for CodeTyphon Studio ==============================
  procedure AssignFromFPImageBitmap(TargetBitmap: TCustomBitmap32; SrcGraphic: TFPImageBitmap);
    var xBmp:TLazIntfImage;
        I: integer;
        H, W : integer;
       ...
       A,R,G,B:byte;
  begin
  xBmp:=SrcGraphic.CreateIntfImage;
  TargetBitmap.SetSize(xBmp.Width, xBmp.Height);

  if SrcGraphic.Transparent then
   begin
     For H:=0 to xBmp.Height-1 do
     Begin
      Line :=xBmp.GetDataLineStart(h);
        For W:=0 to xBmp.Width-1 do
        Begin  // With Transparent Color
         c:=Line^;
         TargetBitmap.Bits[W+(H*xBmp.Width)] :=c;
         inc(Line);
        End;
     End;
   end else
     ...
     Begin
      Line :=xBmp.GetDataLineStart(h);
        ...
        Begin   // No Transparent Color
          ...
          TargetBitmap.Bits[W+(H*Width)] :=color32(B,G,R);
          inc(Line);
        End;
     End;
    end;
...
{$ENDIF}
  //=== ct9999 for CodeTyphon Studio =================
    else if SrcGraphic is TFPImageBitmap then
      AssignFromFPImageBitmap(TargetBitmap, TFPImageBitmap(SrcGraphic))
  //===================================================
    else
...
procedure TCustomBitmap32.SwapRedBlue;
var
  n:    integer;
  temp: longword;
  p:    PLongword;
...
begin
  ...
  try
   ...
   {$IF defined(LCLGtk) or defined(LCLGtk2)}
     if SameText(ExtractFileExt(FileName),'.png')=true then
     SwapRedBlue;
   {$IFEND}
