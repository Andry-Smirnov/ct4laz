...
procedure TCustomColorPicker.WMEraseBkgnd(var Message:TLmEraseBkgnd);
begin
  Message.Result := 1;
end;

procedure TCustomColorPicker.WMGetDlgCode(var Msg:TLMessage);
begin
  with Msg do
...
begin
  ...
  SetInitialBounds(0,0,140,32);  //--- ct9999 ------
end;
...
//------------------------------------
    ...
    SetLength(Polygon, 3);
//--- ct9999 ---- Give ERROR on 64 Bits OS on move left left
    Polygon[0] := FloatPoint(1+3 + Value * (Width - 6), Height - BorderOffset - 5);
...
//--- ct9999 ---- Give ERROR on 64 Bits OS on move left left
      Polygon[0] := FloatPoint(1+3 + Value * (Width - 6), ValueRect.Bottom - BorderOffset - 5);
      Polygon[1] := FloatPoint(Polygon[0].X - 3, Polygon[0].Y + 5);
      Polygon[2] := FloatPoint(Polygon[0].X + 3, Polygon[0].Y + 5);
      RenderPolygon;
