...
uses
  ...
  GR32_AllStrings,
  GR32, GR32_Backends, GR32_Backends_Generic,
...
begin
  ...
  begin
    ...
    try
      ...
      OldFont := SelectObject(DC, Font.Reference.Handle);   //=== ct9999 ==========
...
begin
  ...
  begin
    SelectObject(Handle, Font.Reference.Handle);      //=== ct9999 ===========
    SetTextColor(Handle, ColorToRGB(Font.Color));
    SetBkMode(Handle, TRANSPARENT);
    FFontHandle := Font.Reference.Handle;             //=== ct9999 ===========
  end
  else
  begin
...
begin
  R := FloatRect(X, Y, X, Y);
  GR32_Text_LCL_Win.TextToPath(Font.Reference.Handle, Path, R, Text, 0);  //=== ct9999 ==========
end;

procedure TLCLBackend.TextToPath(Path: TCustomPath; const DstRect: TFloatRect;
  const Text: WideString; Flags: Cardinal);
begin
  GR32_Text_LCL_Win.TextToPath(Font.Reference.Handle, Path, DstRect, Text, Flags);  //=== ct9999 ==========
end;

function TLCLBackend.MeasureText(const DstRect: TFloatRect;
  const Text: WideString; Flags: Cardinal): TFloatRect;
begin
  Result := GR32_Text_LCL_Win.MeasureText(Font.Reference.Handle, DstRect, Text, Flags);   //=== ct9999 ==========
end;
