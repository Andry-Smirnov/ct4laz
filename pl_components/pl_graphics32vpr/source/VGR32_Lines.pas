...
type
  ...
  TCustomArrowHeadProc = function(tipPt, tailPt: TFixedPoint; HeadSize, PenWidth: single;
  ...
  public
    ...
    property CustomProc: TCustomArrowHeadProc read fCustomProc write SetCustomProc;
  end;
  ...
  private
    ...
    procedure DrawGradientHorz(bitmap: TBitmap32; penWidth: single; const colors: array of TColor32; edgeColor: TColor32 = $00000000);
    procedure DrawGradientVert(bitmap: TBitmap32; penWidth: single; const colors: array of TColor32; edgeColor: TColor32 = $00000000);
  protected
  ...
  public
    ...
    function AddPoints(const newPts: array of TFixedPoint; ToEnd: boolean = True): cardinal; overload;
    function AddPoints(var startPt: TFixedPoint; Count: integer; ToEnd: boolean = True): cardinal; overload;
    ...
    procedure Draw(bitmap: TBitmap32; penWidth: single; dashPattern: TArrayOfFloat; color: TColor32; edgeColor: TColor32 = $00000000); overload;

    //draw a line using a color gradient at the specified angle ...
    procedure DrawGradient(bitmap: TBitmap32; penWidth: single; const colors: array of TColor32; angle_degrees: integer;
      edgeColor: TColor32 = $00000000);
...
function InflatePoints(const pts: TArrayOfFixedPoint; delta: single; closed: boolean): TArrayOfFixedPoint; overload;
function InflatePoints(const polyPts: TArrayOfArrayOfFixedPoint; delta: single; closed: boolean): TArrayOfArrayOfFixedPoint; overload;
function InflatePoints(const pts: TArrayOfFloatPoint; delta: single; closed: boolean): TArrayOfFloatPoint; overload;
function InflatePoints(const polyPts: TArrayOfArrayOfFloatPoint; delta: single; closed: boolean): TArrayOfArrayOfFloatPoint; overload;

////////////////////////////////////////////////////////////////////////////////
// Helper drawing functions ...
////////////////////////////////////////////////////////////////////////////////

procedure SimpleLine(bitmap: TBitmap32; const pts: array of TFixedPoint; color: TColor32; Width: single; closed: boolean = False); overload;

procedure SimpleLine(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFixedPoint; color: TColor32; Width: single; closed: boolean); overload;
...
type
  ...
  public
    procedure Grow(const pts: array of TFixedPoint; lineWidth: single; joinStyle: TJoinStyle; miterLimit: single; isClosed: boolean);
  end;
...
procedure SimpleLine(bitmap: TBitmap32; const pts: array of TFixedPoint; color: TColor32; Width: single; closed: boolean = False);
var
  i, len: integer;
...
function OutlineClosedPoints(const pts: array of TFixedPoint; lineWidth: single; PerimeterPointsOnly: boolean = False): TArrayOfFixedPoint;
var
...
begin
  ...
  begin
    if IsClockwise(leftPoints) then
      Result := Copy(leftPoints, 0, length(leftPoints))
    else
...
function InflatePoints(const pts: TArrayOfFixedPoint; delta: single; closed: boolean): TArrayOfFixedPoint;
var
...
function InflatePoints(const polyPts: TArrayOfArrayOfFixedPoint; delta: single; closed: boolean): TArrayOfArrayOfFixedPoint;
var
  i: integer;
begin
  Result := polyPts;
  if (delta = 0) or (length(polyPts) = 0) then
    exit;
    ...
    try
      ...
      begin
        ...
        if not closed then
          ...
          Result[i] := GetRightPoints
        else
          Result[i] := GetLeftPoints;
      end;
    finally
      Free;
    end;
...
function InflatePoints(const pts: TArrayOfFloatPoint; delta: single; closed: boolean): TArrayOfFloatPoint;
var
  polyPts: TArrayOfArrayOfFixedPoint;
begin
  setlength(polyPts, 1);
...
function BuildDashedLine(const pts: TArrayOfFixedPoint; dashes: TArrayOfFloat; closed: boolean = False; dashOffset: TFloat = 0;
  lineWidth: TFloat = 1): TArrayOfArrayOfFixedPoint;
var
  ...
  begin
    Result := (abs(p1.X - p2.X) < epsilon * FixedOne) and (abs(p1.Y - p2.Y) < epsilon * FixedOne);
  end;

begin
  v := 0;
  epsilon := lineWidth / 2;
  if epsilon < 0.00001 then
    epsilon := 0.00001;
  ...
  begin
    if dashes[i] > maxDash then
      maxDash := dashes[i]
    else if dashes[i] < minDash then
      minDash := dashes[i];
  end;
...
function SegmentIntersect(const p1, p2, p3, p4: TFixedPoint; out IntersectPoint: TFixedPoint): boolean;
var
  m1, b1, m2, b2: TFloat;
begin
  Result := False;
...
function GrowDashedLine(const Lines: TArrayOfArrayOfFixedPoint; Width: TFloat): TArrayOfArrayOfFixedPoint;
var
  i, j, highL: integer;
...
begin
  ...
  begin
    ...
    with normals[0] do
      OffsetPoint(lPts[0], X * Width, Y * Width);
    with normals[0] do
      OffsetPoint(rPts[0], -X * Width, -Y * Width);
    with normals[highL - 1] do
      OffsetPoint(lPts[highL * 2 - 1], X * Width, Y * Width);
    with normals[highL - 1] do
      OffsetPoint(rPts[highL * 2 - 1], -X * Width, -Y * Width);
    ...
    begin
      ...
      with normals[j - 1] do
        OffsetPoint(lPts[j * 2 - 1], X * Width, Y * Width);
      with normals[j] do
        OffsetPoint(lPts[j * 2], X * Width, Y * Width);
      ...
      begin
        ...
        else if (highL = 2) and SegmentIntersect(lPts[0], rPts[0], rPts[3], lPts[3], pt1) then
        begin
          lPts[0] := pt1;
          lPts[1] := pt1;
          lPts[2] := pt1;
        ...
        begin
          if ((pt1.X >= lPts[j * 2 - 2].X) = (pt1.X <= lPts[j * 2 - 1].X)) and ((pt1.Y >= lPts[j * 2 - 2].Y) = (pt1.Y <= lPts[j * 2 - 1].Y)) then
            lPts[j * 2 - 1] := pt1
          else
            lPts[j * 2 - 1] := lPts[j * 2 - 2];
          if ((pt1.X >= lPts[j * 2].X) = (pt1.X <= lPts[j * 2 + 1].X)) and ((pt1.Y >= lPts[j * 2].Y) = (pt1.Y <= lPts[j * 2 + 1].Y)) then
            lPts[j * 2] := pt1
          else
            lPts[j * 2] := lPts[j * 2 - 1];
        end;
      ...
      with normals[j - 1] do
        OffsetPoint(rPts[j * 2 - 1], -X * Width, -Y * Width);
      with normals[j] do
        OffsetPoint(rPts[j * 2], -X * Width, -Y * Width);
      ...
      begin
        ...
        begin
          if ((pt1.X > rPts[j * 2 - 2].X) = (pt1.X < rPts[j * 2 - 1].X)) and ((pt1.Y > rPts[j * 2 - 2].Y) = (pt1.Y < rPts[j * 2 - 1].Y)) then
            rPts[j * 2 - 1] := pt1
          else
            rPts[j * 2 - 1] := rPts[j * 2 - 2];
          if ((pt1.X > rPts[j * 2].X) = (pt1.X < rPts[j * 2 + 1].X)) and ((pt1.Y > rPts[j * 2].Y) = (pt1.Y < rPts[j * 2 + 1].Y)) then
            rPts[j * 2] := pt1
          else
            rPts[j * 2] := rPts[j * 2 - 1];
        end;
...
procedure TGrowBase.Grow(const pts: array of TFixedPoint; lineWidth: single; joinStyle: TJoinStyle; miterLimit: single; isClosed: boolean);
var
  ...
  function GetPerpendicularPt(const pt: TFixedPoint; const normal: TFloatPoint; dist: single): TFixedPoint; overload;
  begin
    with FloatPoint(pt) do
      Result := FixedPoint(X + normal.X * dist, Y + normal.Y * dist);
  end;

  function GetPerpendicularPt(const pt, normal: TFloatPoint; dist: single): TFixedPoint; overload;
  begin
    Result := FixedPoint(pt.X + normal.X * dist, pt.Y + normal.Y * dist);
  end;

  function GetVectorPt(const pt: TFixedPoint; const normal: TFloatPoint; dist: single): TFixedPoint;
  begin
  ...
  begin
    ...
    //used here to find if an angle is greater than or less than 180Ί.
    angleIsConcave := (N1.X * N2.Y - N2.X * N1.Y) * dist < 0;
    //(N1.X * N2.X + N1.Y * N2.Y) == unit normal "dot product" == cos(angle)
    //http://en.wikipedia.org/wiki/Dot_product
    //R = 1 + cos(angle). 0 <= R <= 2.
    //R --> 0 as angle --> -180Ί or as angle --> 180Ί.
    R := 1 + (N1.X * N2.X + N1.Y * N2.Y); //1 - cos(ί)  (range: 0 <= R <= 2)
  ...
  begin
    angleIsConcave := (N1.X * N2.Y - N2.X * N1.Y) * dist < 0;
    //R --> 0 as angle --> -180Ί and as angle --> 180Ί.
...
begin
  if (length(pts) < 2) or (LineWidth < 1) then
    exit;
  ...
  begin
    miterLimit := 1;
    if joinStyle = jsMitered then
      joinStyle := jsBevelled;
  end;

  //MiterLimit = sqrt(2/(1 - cos(ί)))
  //Sqr(MiterLimit) = 2/(1 - cos(ί))
  //1 - cos(ί) = 2/Sqr(MiterLimit) = RMin;
  RMin := 2 / sqr(miterLimit);

  //make Normals ...
  highI := high(pts);
  if isClosed and IsDuplicatePoint(pts[0], pts[highI]) then
    Dec(highI);
...
begin
  fCustomProc := Value;
  if not assigned(Value) and (fStyle = asCustom) then
    fStyle := asNone;
  fOwnerLine32.ForceRebuild;
end;
//------------------------------------------------------------------------------
...
begin
  with fOwnerLine32 do
    Result := ((length(fLinePoints) >= 2) and (fEndStyle <> esClosed)) and
      ((Style in [asThreePoint, asFourPoint, asSquare, asDiamond, asCircle]) or ((Style = asCustom) and assigned(fCustomProc)));
end;
//------------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        cumulativeDist := cumulativeDist + DistBetweenPoints(fLinePoints[fBaseIdx], fLinePoints[fBaseIdx + 1]);
        if cumulativeDist > totalDist then
          break;
        Inc(fBaseIdx);
      end;
      ...
      fBasePoint := GetPointAtAngleFromPoint(fLinePoints[0], totalDist, GetAngleOfPt2FromPt1(fLinePoints[0], fLinePoints[fBaseIdx + 1]));
    end
    else
    begin
      while (fBaseIdx > 1) do
      ...
      fBasePoint := GetPointAtAngleFromPoint(fLinePoints[endI], totalDist, GetAngleOfPt2FromPt1(fLinePoints[endI], fLinePoints[fBaseIdx - 1]));
    end;
  end;
end;
//------------------------------------------------------------------------------
...
function GetArrowHeadPoints(tipPt, tailPt: TFixedPoint; HeadSize, PenWidth: single; ArrowHeadStyle: TArrowHeadStyle): TArrayOfFixedPoint;
var
  angle, d: single;
  floatPt: TFloatPoint;
  fr: TFloatRect;
  ...
  if GetTipAndBase then
    case Style of
      asThreePoint, asFourPoint, asSquare, asDiamond, asCircle:
        Result := GetArrowHeadPoints(fTipPoint, fBasePoint, Size, fOwnerLine32.fLineWidth, Style);
      asCustom:
        if assigned(CustomProc) then
          Result := CustomProc(fTipPoint, fBasePoint, Size, fOwnerLine32.fLineWidth, Style);
    end;
end;
//------------------------------------------------------------------------------
...
var
  pts: TArrayOfFixedPoint;
begin
  if (Style = asNone) or (Size < 4) or not assigned(bitmap) then
    exit;
  pts := GetPoints;
  if length(pts) = 0 then
    exit;
...
begin
  ...
  begin
    ...
    if (cnt > 0) and IsDuplicatePoint(pts[i], fLinePoints[cnt - 1]) then
      continue;
...
function TLine32.AddPoints(const newPts: array of TFixedPoint; ToEnd: boolean = True): cardinal;
var
...
begin
  ...
  begin
    ...
    while (bottom <= top) and IsDuplicatePoint(newPts[bottom], Pt) do
      Inc(bottom);
    if bottom > top then
      exit;
  ...
  begin
    Pt := fLinePoints[0];
    while (top >= bottom) and IsDuplicatePoint(newPts[top], Pt) do
      Dec(top);
    if bottom > top then
      exit;
    cnt := top - bottom + 1;
...
function TLine32.AddPoints(var startPt: TFixedPoint; Count: integer; ToEnd: boolean = True): cardinal;
var
  i: integer;
...
begin
  ...
  Result := vgr32_misc.GetBoundsFixedRect(fpa);
end;
//------------------------------------------------------------------------------
...
begin
  if (fStartArrow.Style <> asNone) then
    fStartArrow.Draw(bitmap);
  if (fEndArrow.Style <> asNone) then
    fEndArrow.Draw(bitmap);
end;
//------------------------------------------------------------------------------

procedure TLine32.Draw(bitmap: TBitmap32; penWidth: single; color: TColor32; edgeColor: TColor32 = $00000000);
var
  pts: TArrayOfFixedPoint;
begin
  if not assigned(bitmap) or (length(fLinePoints) < 2) then
    exit;
...
procedure TLine32.Draw(bitmap: TBitmap32; penWidth: single; pattern: TBitmap32; edgeColor: TColor32 = $00000000);
var
  filler: TBitmapPolygonFiller;
  pts: TArrayOfFixedPoint;
begin
  if not assigned(bitmap) or not assigned(pattern) or (length(fLinePoints) < 2) then
    exit;
...
procedure TLine32.Draw(bitmap: TBitmap32; penWidth: single; filler: TCustomPolygonFiller; edgeColor: TColor32 = $00000000);
var
  pts: TArrayOfFixedPoint;
begin
  if not assigned(bitmap) or not assigned(filler) or (length(fLinePoints) < 2) then
    exit;
  fPolygon32.Clear;
  pts := GetOutline(penWidth);
  if length(pts) = 0 then
...
procedure TLine32.Draw(bitmap: TBitmap32; penWidth: single; colors: array of TColor32; StippleStep: single);
var
  pts: TArrayOfFixedPoint;
  ...
  procedure StippleFill(topLeft, topRight, bottomLeft, bottomRight: TFixedPoint; Count: integer; stipOffsetX, stipOffsetY: single);
  var
    j: integer;
  ...
  begin
    ...
    stipCntr := stipCntr + (stipOffsetX * cosAng - stipOffsetY * sinAng) * FixedToFloat * bitmap.StippleStep;
    ...
    try
      ...
      begin
        ...
        begin
          endPt := pts[0];
          with item[0]^ do
            angle := -GetAngleOfPt2FromPt1(left[0], right[0]);
        ...
        begin
          ...
          with item[Count - 1]^ do
            angle := -GetAngleOfPt2FromPt1(right[0], left[0]);
          roundEndPts := BuildArc(FloatPoint(endPt), angle, angle + rad180, penWidth / 2 - 0.5);
          fPolygon32.Clear;
          fPolygon32.AddPoints(roundEndPts[0], length(roundEndPts));
          fPolygon32.DrawFill(bitmap, stippleColor);
...
procedure TLine32.Draw(bitmap: TBitmap32; penWidth: single; dashPattern: TArrayOfFloat; color: TColor32; edgeColor: TColor32 = $00000000);
var
  pts: TArrayOfFixedPoint;
  ppts: TArrayOfArrayOfFixedPoint;
  i: integer;
...
procedure TLine32.DrawGradientHorz(bitmap: TBitmap32; penWidth: single; const colors: array of TColor32; edgeColor: TColor32 = $00000000);
var
  i, j: integer;
  dx: single;
  p: TArrayOfColor32;
...
procedure TLine32.DrawGradientVert(bitmap: TBitmap32; penWidth: single; const colors: array of TColor32; edgeColor: TColor32 = $00000000);
var
  i, j: integer;
  dy: single;
  c: TColor32;
...
procedure TLine32.DrawGradient(bitmap: TBitmap32; penWidth: single; const colors: array of TColor32; angle_degrees: integer;
  edgeColor: TColor32 = $00000000);
var
  bmp, bmp2: TBitmap32;
  rec, rec2, rec3: TRect;
...
begin
  len := length(colors);
  if not assigned(bitmap) or (Length(fLinePoints) < 2) or (len = 0) then
    exit;
  ...
  rec := vgr32_misc.GetBoundsRect(pts);
  ...
  rec2 := vgr32_misc.GetBoundsRect(rotatedPts);
...
begin
  if penWidth > 0 then
    SetWidth(penWidth);
  Build;
  if (length(fLeftPoints) = 0) and (length(fRightPoints) = 0) then
    exit;
  startPts := nil;
  endPts := nil;
  if not fStartArrow.IsNeeded then
  begin
    ...
    else if (fEndStyle = esSquared) then
    begin
  ...
  setLength(Result, length(fLeftPoints) + length(fRightPoints) + length(startPts) + length(endPts));
...
begin
  ...
  begin
    ...
    len := length(Result);
    if (len < 3) or IsDuplicatePoint(Result[0], Result[len - 1]) then
      exit;
...
begin
  Result := htNone;
  if length(fLinePoints) < 2 then
    exit;
  lineOutline := GetOutline(penWidth);
  if length(lineOutline) = 0 then
    exit;
...
begin
  if (fLeftPoints <> nil) or (fRightPoints <> nil) or //ie do only once
    (length(fLinePoints) < 2) or (fLineWidth < 1) then
    exit;
