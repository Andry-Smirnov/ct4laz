...
uses
  ...
  VGR32_PolygonsEx, VGR32_Clipper;
...
procedure ConcatenatePoints(var pts: TArrayOfFixedPoint; const extras: TArrayOfFixedPoint); overload;
procedure ConcatenatePoints(var pts: TArrayOfFixedPoint; const extras: array of TFixedPoint); overload;
procedure ConcatenatePolyPoints(var polyPts: TArrayOfArrayOfFixedPoint; const extras: TArrayOfArrayOfFixedPoint);
...
function AAFloatToAAFixed(ppts: TArrayOfArrayOfFloatPoint): TArrayOfArrayOfFixedPoint;
...
function ClosestPointOnLine(const pt, lnA, lnB: TFloatPoint; ForceResultBetweenLinePts: boolean): TFloatPoint;
function DistOfPointFromLine(const pt, lnA, lnB: TFloatPoint; ForceResultBetweenLinePts: boolean): TFloat; overload;
function DistOfPointFromLine(const pt, lnA, lnB: TFixedPoint; ForceResultBetweenLinePts: boolean): TFloat; overload;
function IntersectionPoint(const ln1A, ln1B, ln2A, ln2B: TFixedPoint; out IntersectPoint: TFixedPoint): boolean;

function RotatePoint(pt, origin: TFixedPoint; const radians: single): TFixedPoint;
function RotatePoints(const pts: array of TFixedPoint; origin: TFixedPoint; radians: single): TArrayOfFixedPoint; overload;
function RotatePoints(const pts: array of TFixedPoint; radians: single): TArrayOfFixedPoint; overload;
procedure RotatePolyPoints(var pts: TArrayOfArrayOfFixedPoint; origin: TFixedPoint; radians: single);
...
function CreateMaskFromPolygon(bitmap: TBitmap32; const polygon: TArrayOfFixedPoint): TBitmap32; overload;
function CreateMaskFromPolygon(bitmap: TBitmap32; const polygons: TArrayOfArrayOfFixedPoint): TBitmap32; overload;
...
function GetPtOnEllipseFromAngleEccentric(const ellipseRect: TFloatRect; eccentric_angle_radians: single): TFloatPoint;

function GetPointsAroundEllipse(const ellipseRect: TFloatRect; PointCount: integer): TArrayOfFixedPoint;
...
function GetArcPoints(const ellipseRect: TFloatRect; startPt, endPt: TFloatPoint): TArrayOfFixedPoint; overload;

function GetArcPoints(const ellipseRect: TFloatRect; start_degrees, end_degrees: single): TArrayOfFixedPoint; overload;

function GetArcPointsEccentric(const ellipseRect: TFloatRect; start_eccentric, end_eccentric: single): TArrayOfFixedPoint;

function GetPiePoints(const ellipseRect: TFloatRect; startPt, endPt: TFloatPoint): TArrayOfFixedPoint; overload;

function GetPiePoints(const ellipseRect: TFloatRect; start_degrees, end_degrees: single): TArrayOfFixedPoint; overload;

function GetPiePointsEccentric(const ellipseRect: TFloatRect; start_eccentric, end_eccentric: single): TArrayOfFixedPoint;

function GetRoundedRectanglePoints(const rect: TFloatRect; roundingPercent: cardinal): TArrayOfFixedPoint;

function GetBalloonedEllipsePoints(const ellipseRect: TFloatRect; balloonPos: TBalloonPos): TArrayOfFixedPoint;

function GetPipeCornerPoints(rec: TFloatRect; pipeWidth: single; corner: TCorner): TArrayOfFixedPoint;

function GetStarPoints(const Center: TFixedPoint; PointCount: integer; radius1, radius2: single): TArrayOfFixedPoint;
...
function BuildDashedLine(const Points: TArrayOfFixedPoint; const DashArray: array of TFloat; DashOffset: TFloat = 0): TArrayOfArrayOfFixedPoint;
...
procedure SimpleLine(bitmap: TBitmap32; const pts: array of TFixedPoint; color: TColor32; closed: boolean = False); overload;
procedure SimpleLine(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFixedPoint; color: TColor32; closed: boolean); overload;

//fill a set of closed points with a color (with 1px width edge) ...
procedure SimpleFill(bitmap: TBitmap32; pts: array of TFixedPoint; edgeColor, fillColor: TColor32; aFillMode: TPolyFillMode = pfWinding); overload;
procedure SimpleFill(bitmap: TBitmap32; const pts: array of TFloatPoint; edgeColor, fillColor: TColor32;
  aFillMode: TPolyFillMode = pfWinding); overload;
procedure SimpleFill(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; edgeColor, fillColor: TColor32;
  aFillMode: TPolyFillMode = pfWinding); overload;

//fill a set of closed points with a bitmap32 pattern (with 1px width edge) ...
procedure SimpleFill(bitmap: TBitmap32; const pts: array of TFixedPoint; edgeColor: TColor32; pattern: TBitmap32;
  aFillMode: TPolyFillMode = pfWinding); overload;

procedure SimpleFill(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; edgeColor: TColor32; pattern: TBitmap32;
  aFillMode: TPolyFillMode = pfWinding); overload;

procedure SimpleFill(bitmap: TBitmap32; const pts: TArrayOfArrayOfFloatPoint; edgeColor, fillColor: TColor32;
  aFillMode: TPolyFillMode = pfWinding); overload;

//fill a set of closed points with a color gradient at the specified angle ...
procedure SimpleGradientFill(bitmap: TBitmap32; pts: array of TFixedPoint; edgeColor: TColor32; const colors: array of TColor32;
  angle_degrees: integer); overload;
procedure SimpleGradientFill(bitmap: TBitmap32; pts: TArrayOfArrayOfFixedPoint; edgeColor: TColor32;
  const colors: array of TColor32; angle_degrees: integer); overload;

procedure SimpleStippleFill(bitmap: TBitmap32; pts: array of TFixedPoint; const colors: array of TColor32; step: single; angle_degrees: integer);

procedure SimpleRadialFill(bitmap: TBitmap32; const pt: TFixedPoint; radius: single; const colors: array of TColor32); overload;
procedure SimpleRadialFill(bitmap: TBitmap32; rec: TFloatRect; const colors: array of TColor32); overload;

procedure SimpleRadialFill(bitmap: TBitmap32; const pts: array of TFixedPoint; const colors: array of TColor32;
  angle_degrees: integer = 0); overload;
procedure SimpleRadialFill(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFixedPoint; const colors: array of TColor32;
  angle_degrees: integer = 0); overload;
...
procedure SimpleShadow(bitmap: TBitmap32; const pts: TArrayOfFixedPoint; dx, dy, fadeRate: integer; shadowColor: TColor32;
  closed: boolean = False; NoInteriorBleed: boolean = False); overload;
procedure SimpleShadow(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; dx, dy, fadeRate: integer;
  shadowColor: TColor32; closed: boolean = False; NoInteriorBleed: boolean = False); overload;

procedure Simple3D(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFloatPoint; dx, dy, fadeRate: integer;
  topLeftColor, bottomRightColor: TColor32); overload;
procedure Simple3D(bitmap: TBitmap32; const pts: TArrayOfFixedPoint; dx, dy, fadeRate: integer; topLeftColor, bottomRightColor: TColor32); overload;
procedure Simple3D(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; dx, dy, fadeRate: integer;
  topLeftColor, bottomRightColor: TColor32); overload;

//nb: Using semiopaque colors, SimpleBevel can be an easy way to add watermarks
procedure SimpleBevel(bitmap: TBitmap32; ppts: TArrayOfArrayOfFixedPoint; strokeWidth: integer; lightClr, darkClr: TColor32;
...
procedure TPolygon32Ex.Draw(Bitmap: TBitmap32; OutlineColor: TColor32; Filler: TCustomPolygonFiller);
var
  tmp: TArrayOfArrayOfFloatPoint;
begin
  tmp := AAFixedToAAFloat(fPPts);
...
begin
  ...
  for i := 0 to highPts do
    Result[i] := pts[highPts - i];
end;
//------------------------------------------------------------------------------
...
procedure ConcatenatePoints(var pts: TArrayOfFixedPoint; const extras: TArrayOfFixedPoint);
var
...
procedure ConcatenatePoints(var pts: TArrayOfFixedPoint; const extras: array of TFixedPoint);
var
  i, ptsCnt, extrasCnt: integer;
begin
  ptsCnt := length(pts);
...
begin
  ...
  setlength(Result, len);
  for i := 0 to len - 1 do
    Result[i] := MakeArrayOfFloatPoints(a[i]);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  setlength(Result, len);
  for i := 0 to len - 1 do
    Result[i] := MakeArrayOfFixedPoints(a[i]);
end;
//------------------------------------------------------------------------------
...
begin
  Result :=
    (abs(rec1.Left - rec2.Left) < tolerance) and (abs(rec1.Top - rec2.Top) < tolerance) and
    (abs(rec1.Right - rec2.Right) < tolerance) and (abs(rec1.Bottom - rec2.Bottom) < tolerance);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  for i := 1 to len - 1 do
  begin
    if IsDuplicatePoint(Result[j - 1], pts[i]) then
      continue;
    Result[j] := pts[i];
    Inc(j);
  end;
...
function GetPointAtAngleFromPoint(const pt: TFixedPoint; const dist, radians: single): TFixedPoint;
var
...
begin
  ...
  begin
    q := ((pt.X - lnA.X) * (lnB.X - lnA.X) + (pt.Y - lnA.Y) * (lnB.Y - lnA.Y)) / (sqr(lnB.X - lnA.X) + sqr(lnB.Y - lnA.Y));
    if ForceResultBetweenLinePts then
      constrain(q, 0, 1);
...
function DistOfPointFromLine(const pt, lnA, lnB: TFloatPoint; ForceResultBetweenLinePts: boolean): TFloat;
var
  cpol: TFloatPoint;
begin
...
function DistOfPointFromLine(const pt, lnA, lnB: TFixedPoint; ForceResultBetweenLinePts: boolean): TFloat;
begin
  Result := DistOfPointFromLine(FloatPoint(pt), FloatPoint(lnA), FloatPoint(lnB), ForceResultBetweenLinePts);
end;
//------------------------------------------------------------------------------
...
function RotatePoints(const pts: array of TFixedPoint; origin: TFixedPoint; radians: single): TArrayOfFixedPoint;
var
  i: integer;
  tmp: TFixedPoint;
  cosAng, sinAng: single;
...
function RotatePoints(const pts: array of TFixedPoint; radians: single): TArrayOfFixedPoint;
var
  i: integer;
  cosAng, sinAng: single;
begin
...
begin
  ...
  for I := 0 to High(Pts) do
  begin
    Result := Result xor (((Pt.Y >= iPt^.Y) xor (Pt.Y >= jPt^.Y)) and (Pt.X - iPt^.X < MulDiv(jPt^.X - iPt^.X,
      Pt.Y - iPt^.Y, jPt^.Y - iPt^.Y)));
    jPt := iPt;
    Inc(iPt);
  end;
end;
...
var
  ...
  begin
    if (Pts[j].Y = pt.Y) then
      ...
      else if (Pts[i].Y = pt.Y) and ((Pts[j].X > pt.X) = (Pts[i].X < pt.X)) then
      begin
        ptOnPolygonFlag := True;
        exit;
      end;
    if (Pts[i].Y < pt.Y) <> (Pts[j].Y < pt.Y) then
      ...
      begin
        if (Pts[j].X >= pt.X) then
        begin
          if Pts[j].Y > Pts[i].Y then
            Inc(windingNum)
          else
            Dec(windingNum);
        end
        else if (det(i, j) > 0) = (Pts[j].Y > Pts[i].Y) then
        begin
          if Pts[j].Y > Pts[i].Y then
            Inc(windingNum)
          else
      ...
      else if (Pts[j].X >= pt.X) and ((det(i, j) > 0) = (Pts[j].Y > Pts[i].Y)) then
      begin
        if Pts[j].Y > Pts[i].Y then
          Inc(windingNum)
        else
...
begin
  Result := False;
  highI := high(Pts);
  if (highI < 2) or (pt.X = Pts[0].X) and (pt.Y = Pts[0].Y) then
    exit;
...
begin
  Result := rec1;
  if rec2.Left < Result.Left then
    Result.Left := rec2.Left;
  if rec2.Right > Result.Right then
    Result.Right := rec2.Right;
  if rec2.Top < Result.Top then
    Result.Top := rec2.Top;
  if rec2.Bottom > Result.Bottom then
    Result.Bottom := rec2.Bottom;
end;
//------------------------------------------------------------------------------
...
var
  ...
  begin
    repeat
      ...
      P := list^[(l + r) shr 1];
      repeat
        while list^[i].cotangent_value > P.cotangent_value do
          Inc(i);
        while list^[j].cotangent_value < P.cotangent_value do
          Dec(j);
        if i <= j then
        begin
          T := list^[i];
...
begin
  ...
  m := 0;
  for i := 1 to len - 1 do
    if polygon[i].Y < polygon[m].Y then
      continue
    else if (polygon[i].Y > polygon[m].Y) or (polygon[i].X < polygon[m].X) then
      m := i;
  ...
  begin
    if i = m then
      sortArray[i].pt := FloatPoint(polygon[0])
    else
      sortArray[i].pt := FloatPoint(polygon[i - 1]);
    if abs(sortArray[i].pt.Y - sortArray[1].pt.Y) < 0.0001 then
      sortArray[i].cotangent_value := -MaxSingle
    else
      sortArray[i].cotangent_value := (sortArray[i].pt.X - sortArray[1].pt.X) / (sortArray[i].pt.Y - sortArray[1].pt.Y);
  end;
  ...
  for i := 3 to len do
  begin
    while IsClockwise(sortArray[m - 1].pt, sortArray[m].pt, sortArray[i].pt) do
      Dec(m);
    Inc(m);
...
function CreateMaskFromPolygon(bitmap: TBitmap32; const polygons: TArrayOfArrayOfFixedPoint): TBitmap32;
var
  i, highI: integer;
begin
...
begin
  ...
  if (bitmap.DrawMode = dmOpaque) or (bitmap.Width <> Mask.Width) or (bitmap.Height <> Mask.Height) then
    exit;
...
begin
  if not assigned(originalBmp) or not assigned(maskBmp) or (originalBmp.Width <> modifiedBmp.Width) or
    (originalBmp.Height <> modifiedBmp.Height) or (originalBmp.Height <> maskBmp.Height) or (originalBmp.Height <> maskBmp.Height) then
    exit;
  ...
  begin
    ...
    if invertMask then
      MergeMemEx(origClr^.ARGB, modClr^.ARGB, 255 - mskClr^.B)
    else
...
begin
  ...
  Result[0] := chartPts[0];
  with FloatPoint(chartPts[0]) do
    Result[1] := FixedPoint(X + dx, Y);
  ...
  begin
    with FloatPoint(chartPts[i]) do
      Result[i * 3 - 1] := FixedPoint(X - dx, Y);
  ...
  with FloatPoint(chartPts[ptCnt - 1]) do
    Result[high(Result) - 1] := FixedPoint(X - dx, Y);
...
var
  ...
  begin
    ...
    if abs(p1.x + p3.x - 2 * p2.x) + abs(p2.x + p4.x - 2 * p3.x) + abs(p1.y + p3.y - 2 * p2.y) +
...
begin
  ...
  if (arrayLen < 4) or ((arrayLen - 1) mod 3 <> 0) then
    exit;
...
var
  ...
  begin
    //assess flatness of curve ...
    //http://groups.google.com/group/comp.graphics.algorithms/tree/browse_frm/thread/d85ca902fdbd746e
    if abs(p1.x + p3.x - 2 * p2.x) + abs(p2.x + p4.x - 2 * p3.x) + abs(p1.y + p3.y - 2 * p2.y) + abs(p2.y + p4.y - 2 * p3.y) < cbezier_tolerance then
    begin
      if resultCnt = length(Result) then
        setLength(Result, length(Result) + 128);
      Result[resultCnt] := FixedPoint(p4);
...
begin
  ...
  //first check that the 'control_points' count is valid ...
  arrayLen := length(control_points);
  if (arrayLen < 4) or odd(arrayLen) then
    exit;
...
begin
  //first check that the 'control_points' count is valid ...
  arrayLen := length(control_points);
  if (arrayLen < 3) or ((arrayLen - 1) mod 2 <> 0) then
    exit;
...
begin
  ...
  //x = a * cosΨ; y = b * sinΨ; y/x = tanί;
  //a = x / cosΨ; b = y / sinΨ
  //if a / b = r then a = r * b and x / cosΨ = r * y / sinΨ
  //x * sinΨ / cosψ = r * y
  //x * tanΨ = r * y
  //tanΨ = r * y/x
  //Ψ = arctan(a/b * y/x);
...
function GetArcPoints(const ellipseRect: TFloatRect; startPt, endPt: TFloatPoint): TArrayOfFixedPoint;
var
  c: TFloatPoint;
...
function GetArcPoints(const ellipseRect: TFloatRect; start_degrees, end_degrees: single): TArrayOfFixedPoint;
var
...
function GetPiePoints(const ellipseRect: TFloatRect; startPt, endPt: TFloatPoint): TArrayOfFixedPoint;
var
  len: integer;
begin
...
function GetPiePoints(const ellipseRect: TFloatRect; start_degrees, end_degrees: single): TArrayOfFixedPoint;
var
  start_radians, end_radians: single;
begin
  start_radians := AngleToEccentricAngle(ellipseRect, start_degrees * DegToRad);
...
begin
  ...
  if roundingPercent < 5 then
    roundingPercent := 5
  else if roundingPercent > 90 then
    roundingPercent := 90;
  ...
  begin
    ...
    if (dy < dx) then
    begin
      if dx < (Bottom - Top) * 0.45 then
        dy := dx
      else
        dy := (Bottom - Top) * 0.45;
    end;
    arcs[0] := GetArcPointsEccentric(FloatRect(Left, Bottom - dy * 2, Left + dx * 2, Bottom), rad180, rad270);
    arcs[1] := GetArcPointsEccentric(FloatRect(Right - dx * 2, Bottom - dy * 2, Right, Bottom), rad270, 0);
    arcs[2] := GetArcPointsEccentric(FloatRect(Right - dx * 2, Top, Right, Top + dy * 2), 0, rad90);
    arcs[3] := GetArcPointsEccentric(FloatRect(Left, top, Left + dx * 2, Top + dy * 2), rad90, rad180);
  end;
...
begin
  case balloonPos of
    ...
    bpTopLeft:
      with ellipseRect do
      begin
        Result := GetArcPointsEccentric(FloatRect(left, top, right, bottom), (135 + diamondSize) * DegToRad, (135 - diamondSize) * DegToRad);
        len := length(Result);
    ...
    bpTopRight:
      with ellipseRect do
      begin
        Result := GetArcPointsEccentric(FloatRect(left, top, right, bottom), (45 + diamondSize) * DegToRad, (45 - diamondSize) * DegToRad);
        len := length(Result);
        setlength(Result, len + 1);
    ...
    bpBottomRight:
      with ellipseRect do
      begin
        Result := GetArcPointsEccentric(FloatRect(left, top, right, bottom), (315 + diamondSize) * DegToRad, (315 - diamondSize) * DegToRad);
        len := length(Result);
        setlength(Result, len + 1);
    ...
    bpBottomLeft:
      with ellipseRect do
      begin
        Result := GetArcPointsEccentric(FloatRect(left, top, right, bottom), (225 + diamondSize) * DegToRad, (225 - diamondSize) * DegToRad);
        len := length(Result);
        setlength(Result, len + 1);
...
begin
  ...
  begin
    ...
    if (radius < 5) or (pipeWidth < 1) then
      exit
    else if pipeWidth > radius then
      pipeWidth := radius;
    ...
    case corner of
      cTopLeft:
      begin
        outerRec := FloatRect(Left + delta, Top + delta, Left + delta + (radius - delta) * 2, Top + delta + (radius - delta) * 2);
        innerRec := FloatRect(Left + radius - delta, Top + radius - delta, Left + radius - delta + delta * 2, Top + radius - delta + delta * 2);
      ...
      cTopRight:
      begin
        outerRec := FloatRect(Right - delta - (radius - delta) * 2, Top + delta, Right - delta, Top + delta + (radius - delta) * 2);
        innerRec := FloatRect(Right - radius - delta, Top + radius - delta, Right - radius - delta + delta * 2, Top + radius - delta + delta * 2);
      ...
      cBottomLeft:
      begin
        outerRec := FloatRect(Left + delta, Bottom - delta - (radius - delta) * 2, Left + delta + (radius - delta) * 2, Bottom - delta);
        innerRec := FloatRect(Left + radius - delta, Bottom - radius - delta, Left + radius - delta + delta * 2, Bottom - radius - delta + delta * 2);
      ...
      cBottomRight:
      begin
        outerRec := FloatRect(Right - delta - (radius - delta) * 2, Bottom - delta - (radius - delta) * 2, Right - delta, Bottom - delta);
        innerRec := FloatRect(Right - radius - delta, Bottom - radius - delta, Right - radius - delta + delta * 2, Bottom - radius - delta + delta * 2);
...
begin
  ...
  bottomIdx := 0;
  for i := 1 to highI do
    if pts[i].Y < pts[bottomIdx].Y then
      continue
    else if pts[i].Y > pts[bottomIdx].Y then
      bottomIdx := i
    else if pts[i].X > pts[bottomIdx].X then
      bottomIdx := i;
...
procedure SimpleLine(bitmap: TBitmap32; const pts: array of TFixedPoint; color: TColor32; closed: boolean = False);
var
  i, j: integer;
begin
  j := high(pts);
  ...
  if closed then
    bitmap.LineXS(pts[j].X, pts[j].Y, pts[0].X, pts[0].Y, color);
end;
//------------------------------------------------------------------------------

procedure SimpleLine(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFixedPoint; color: TColor32; closed: boolean);
var
  i: integer;
begin
  for i := 0 to length(ppts) - 1 do
    SimpleLine(bitmap, ppts[i], color, closed);
end;
//------------------------------------------------------------------------------

procedure SimpleFill(bitmap: TBitmap32; pts: array of TFixedPoint; edgeColor, fillColor: TColor32; aFillMode: TPolyFillMode = pfWinding);
begin
  with TPolygon32Ex.Create do
    try
      Closed := True;
...
procedure SimpleFill(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; edgeColor, fillColor: TColor32; aFillMode: TPolyFillMode = pfWinding);
var
  i, len: integer;
begin
  len := length(pts);
...
procedure SimpleFill(bitmap: TBitmap32; const pts: array of TFloatPoint; edgeColor, fillColor: TColor32; aFillMode: TPolyFillMode = pfWinding);
var
  i, highI: integer;
...
procedure SimpleFill(bitmap: TBitmap32; const pts: TArrayOfArrayOfFloatPoint; edgeColor, fillColor: TColor32; aFillMode: TPolyFillMode = pfWinding);
var
  i, len: integer;
  pts2: TArrayOfFixedPoint;
begin
...
procedure SimpleFill(bitmap: TBitmap32; const pts: array of TFixedPoint; edgeColor: TColor32; pattern: TBitmap32;
  aFillMode: TPolyFillMode = pfWinding);
var
  i, len: integer;
...
procedure SimpleGradientFillHorz(bitmap: TBitmap32; pts: TArrayOfArrayOfFixedPoint; edgeColor: TColor32; const colors: array of TColor32);
var
  i, j: integer;
  bmp: TBitmap32;
  rec: TFixedRect;
...
procedure SimpleGradientFillVert(bitmap: TBitmap32; pts: TArrayOfArrayOfFixedPoint; edgeColor: TColor32; const colors: array of TColor32);
var
  i, j: integer;
  bmp: TBitmap32;
  rec: TFixedRect;
...
begin
  ...
  begin
    ...
    for i := 0 to len - 1 do
      reverseColors[i] := colors[len - 1 - i];
  ...
  i := 0;
  len := length(pts);
  while (i < len) and (length(pts[i]) = 0) do
    Inc(i);
  if i = len then
    exit;
  for j := i to len - 1 do
  begin
    if length(pts[j]) = 0 then
      continue;
    if j = i then
  ...
  if (rec.Right = rec.Left) or (rec.Bottom = rec.Top) then
    exit;
  with rec do
  ...
  rec3 := rec2;
  if rec3.Left > rec.Left then
    rec3.Left := rec.Left;
  if rec3.Top > rec.Top then
    rec3.Top := rec.Top;
  if rec3.Right < rec.Right then
    rec3.Right := rec.Right;
  if rec3.Bottom < rec.Bottom then
    rec3.Bottom := rec.Bottom;
...
procedure SimpleRadialFill(bitmap: TBitmap32; const pt: TFixedPoint; radius: single; const colors: array of TColor32);
var
...
begin
  colorLen := length(colors);
  if (radius < 1) or (colorLen = 0) then
    exit;
  if colorLen = 1 then
  begin
  ...
  if rec.Left < 0 then
    rec.Left := 0;
  if rec.Right >= bitmap.Width then
    rec.Right := bitmap.Width - 1;
  if rec.Top < 0 then
    rec.Top := 0;
  if rec.Bottom >= bitmap.Height then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if dstQ1 <> dstQ2 then
            BlendMemEx(outerColor, dstQ2^, M);
          if dstQ1 <> dstQ3 then
            BlendMemEx(outerColor, dstQ3^, M);
...
procedure SimpleRadialFill(bitmap: TBitmap32; rec: TFloatRect; const colors: array of TColor32);
var
  i, j, colorLen: integer;
...
procedure SimpleRadialFill(bitmap: TBitmap32; const pts: array of TFixedPoint; const colors: array of TColor32; angle_degrees: integer = 0);
var
  i, len: integer;
  polyPts: TArrayOfArrayOfFixedPoint;
begin
...
procedure SimpleRadialFill(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFixedPoint; const colors: array of TColor32; angle_degrees: integer = 0);
var
  i, j, len: integer;
...
begin
  ...
  for i := 0 to len - 1 do
    ...
    begin
      //Ψ = arctan2(a*y, b*x); a = x / cosΨ
      if (mp.X = ppts2[i][j].X) or (mp.Y = ppts2[i][j].Y) then
        continue;
...
procedure SimpleShadow(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; dx, dy, fadeRate: integer;
  shadowColor: TColor32; closed: boolean = False; NoInteriorBleed: boolean = False);
var
  i, j, maxD: integer;
  ...
  sc: TColor32;
begin
  if ((dx = 0) and (dy = 0)) or (length(pts) = 0) then
    exit;
  ...
  if fadeRate <= MAXIMUM_SHADOW_FADE then
    dRate := 0.05
  else if fadeRate >= MINIMUM_SHADOW_FADE then
    dRate := 0.95
  else
    dRate := fadeRate / 10;
...
procedure Simple3D(bitmap: TBitmap32; const ppts: TArrayOfArrayOfFloatPoint; dx, dy, fadeRate: integer; topLeftColor, bottomRightColor: TColor32);
var
  i, j: integer;
  ppts2: TArrayOfArrayOfFixedPoint;
begin
...
procedure Simple3D(bitmap: TBitmap32; const pts: TArrayOfFixedPoint; dx, dy, fadeRate: integer; topLeftColor, bottomRightColor: TColor32);
var
  ppts: TArrayOfArrayOfFixedPoint;
begin
  SetLength(ppts, 1);
...
procedure Simple3D(bitmap: TBitmap32; const pts: TArrayOfArrayOfFixedPoint; dx, dy, fadeRate: integer; topLeftColor, bottomRightColor: TColor32);
var
  mask, orig: TBitmap32;
begin
