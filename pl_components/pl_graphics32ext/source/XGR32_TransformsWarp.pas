...
unit XGR32_TransformsWarp;

interface

uses

  SysUtils, Classes, GR32,  GR32_Transforms, GR32_Math, XGR32_Algebra;
...
var
  sY,cY:Single;
begin
  GR32_Math.SinCos(Y*HalfPIS,sY,cY);
  if cY <> 0 then
  begin
    Y := -X / cY;
    X := -FFocal * sY / cY;
...
var
  sX,cX:Single;
begin
  GR32_Math.SinCos(X*HalfPIS,sX,cX);
  if cX <> 0 then
  begin
    X := -Y / cX;
    Y := -FFocal * sX / cX;
