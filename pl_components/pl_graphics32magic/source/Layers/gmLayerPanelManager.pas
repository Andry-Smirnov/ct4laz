...
uses
  ...
  LCLIntf, LCLType, LMessages, SysUtils, Graphics, Forms, Math,
...
begin
  ...
  if PtInRect(LRect, LTestPoint) then
  begin
...
begin
  Result := PtInRect(Self.ClientRect, ARect.TopLeft) or
            PtInRect(Self.ClientRect, ARect.BottomRight);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if PtInRect( LValidArea, Point(X, Y) ) then
        begin
          LPos := Point(X, Y);
        end
        else
