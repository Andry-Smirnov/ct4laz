...
uses    
 {$IFDEF Windows}
  Windows,
 {$ENDIF}
  LCLIntf, LCLType, LMessages,types, Classes, Graphics, Controls,
  gmTypes;
...
procedure ClosestPointOnBezier(const ABezier: TsdBezier; const APoint: TsdPoint; Tol: Double;
                               var A: Double; var Steps: Integer;
                               var ADist: Double; var Closest: TsdPoint);
...
function TgmCurveSegmentsList.SelectAtSingleSegment(const AX, AY: Integer): Boolean;
var
  i, LPobSteps : Integer;
  LBezier      : TsdBezier;
  LPoint       : TsdPoint;
...
begin
  ...
  begin
    ...
    begin
      ...
      if IsPointOnBezier(LBezier, LPoint, LPobSteps, TOLERANCE_OVER_BEZIER) then
      begin
        FCurrentIndex := i;
        FSelected     := True;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          ClosestPointOnBezier(LBezier, LPoint, TOLERANCE_OVER_BEZIER,
                                        A, LSteps, LDist, LClosest);

          // Split beziers using factor A
          SplitBezierWithFactor(LBezier, A, B1, B2);
...
function  IsPointOnBezier(const ABezier: TsdBezier; const APoint: TsdPoint;
                          var Steps: Integer; Tol: Double): Boolean;
begin
  Result:=false;
end;
