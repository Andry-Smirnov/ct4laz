...
unit igGradient_rwPhotoshopGRD;
...
{ GraphicsMagic }
  igCore_rw, igGradient,
  GR32, GR32_LowLevel, igbestream;

const
  PS_GRADIENT_FILE_ID = $52474238; // 8BGR

type
  TgrdPhotoshopReader = class(TigConverter)
  private
    procedure LoadColorStopItem(Node: TigGradientStop);
    procedure LoadAlphaStopItem(Node: TigGradientStop);
    function GetStopItemRGB:TColor;
    function GetStopItemCMYK:TColor;
    //function GetStopItemHSB:TColor;
    procedure SetStopItemHSB(Node : TigGradientStop);
  public
    ...
    procedure LoadItemFromStream(AStream: TStream; AGradient: TigGradientItem); reintroduce;
  end;
...
procedure TgrdPhotoshopReader.LoadFromStream(AStream: TStream;
...
//  LGradients        := TgmGradientCollection(ACollection);
  ...
  for i := 1 to LPShdr.GradientCount do
    LoadItemFromStream(AStream, ACollection.Add as TigGradientItem);
end;

class function TgrdPhotoshopReader.WantThis(AStream: TStream): Boolean;
var
  LFileHeader: TigFileHeader;
begin
  AStream.Read(LFileHeader, SizeOf(LFileHeader));
   
  Result := (LFileHeader.FileID = PS_GRADIENT_FILE_ID) and
            (SwapEndian(LFileHeader.Version.BEMajor) = 5);  //only support ver 5 of PS.GRD
end;

procedure TgrdPhotoshopReader.LoadAlphaStopItem(Node: TigGradientStop);
begin
...
procedure TgrdPhotoshopReader.LoadColorStopItem(Node: TigGradientStop);
begin
  chunkO.chunk := BE_ReadFlag;       //Objc
  swide        := BE_ReadWideString; //""
  ...
  begin
    ...
    TigGradientItem(Node.Collection.Owner).ColorSpace := chunkO.chunk;
    
    if chunkO.chunk = 'RGBC' then
      Node.AsColor := GetStopItemRGB
    else if chunkO.chunk = 'CMYC' then
      Node.AsColor := GetStopItemCMYK
  ...
  begin
    if chunkO.chunk = 'BckC' then
      Node.AsColor := clBackground
    else
    if chunkO.chunk = 'FrgC' then
      Node.AsColor := clDefault;
  end;
...
procedure TgrdPhotoshopReader.LoadItemFromStream(AStream: TStream;
  AGradient: TigGradientItem);
var
  i : Integer;
begin
  chunkO.chunk := BE_ReadFlag;       //Objc
...
procedure TgrdPhotoshopReader.SetStopItemHSB(Node : TigGradientStop);
...
begin
  ...
  Node.AsColor := TColor( B shl 16 + G shl 8 + R);
end;

initialization
  TigGradientList.RegisterConverterReader('GRD', 'Photoshop gradient', 0, TgrdPhotoshopReader);
