...
{$if defined(MSWINDOWS)}  
  cnlibusbFile = 'libusb-1.0.dll';
{$elseif defined(LINUX)}  
  cnlibusbFile = 'libusb-1.0.so.0';
{$ELSE}
   cnlibusbFile = 'libusb-1.0.so
{$ENDIF}
...
//----------------------------------------------------
  ...
  VarUSBLibHandle : TLibHandle = 0;

  Function  USBLib_Init(Const aLibFile:string=''):Boolean;
  Procedure USBLib_Free;
...
Begin
  ...
  if aLibFile='' then
     VarUSBLibHandle := LoadLibrary(cnlibusbFile) else
     VarUSBLibHandle := LoadLibrary(aLibFile);

  if VarUSBLibHandle = 0 then
   begin
    //raise Exception.Create('Couldn''t load dynamic library libusb.');
    exit;
   end;

  pointer(libusb_init) := GetProcedureAddress(VarUSBLibHandle, 'libusb_init');
  pointer(libusb_exit) := GetProcedureAddress(VarUSBLibHandle, 'libusb_exit');
  pointer(libusb_set_debug) := GetProcedureAddress(VarUSBLibHandle, 'libusb_set_debug');
  pointer(libusb_get_version) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_version');
  pointer(libusb_has_capability) := GetProcedureAddress(VarUSBLibHandle, 'libusb_has_capability');
  pointer(libusb_error_name) := GetProcedureAddress(VarUSBLibHandle, 'libusb_error_name');
  pointer(libusb_get_device_list) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_device_list');
  pointer(libusb_free_device_list) := GetProcedureAddress(VarUSBLibHandle, 'libusb_free_device_list');
  pointer(libusb_ref_device) := GetProcedureAddress(VarUSBLibHandle, 'libusb_ref_device');
  pointer(libusb_unref_device) := GetProcedureAddress(VarUSBLibHandle, 'libusb_unref_device');
  pointer(libusb_get_configuration) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_configuration');
  pointer(libusb_get_device_descriptor) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_device_descriptor');
  pointer(libusb_get_active_config_descriptor) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_active_config_descriptor');
  pointer(libusb_get_config_descriptor) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_config_descriptor');
  pointer(libusb_get_config_descriptor_by_value) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_config_descriptor_by_value');
  pointer(libusb_free_config_descriptor) := GetProcedureAddress(VarUSBLibHandle, 'libusb_free_config_descriptor');
  pointer(libusb_get_bus_number) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_bus_number');
  pointer(libusb_get_port_number) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_port_number');
  pointer(libusb_get_parent) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_parent');
  pointer(libusb_get_port_path) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_port_path');
  pointer(libusb_get_device_address) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_device_address');
  pointer(libusb_get_device_speed) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_device_speed');
  pointer(libusb_get_max_packet_size) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_max_packet_size');
  pointer(libusb_get_max_iso_packet_size) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_max_iso_packet_size');
  pointer(libusb_open) := GetProcedureAddress(VarUSBLibHandle, 'libusb_open');
  pointer(libusb_close) := GetProcedureAddress(VarUSBLibHandle, 'libusb_close');
  pointer(libusb_get_device) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_device');
  pointer(libusb_set_configuration) := GetProcedureAddress(VarUSBLibHandle, 'libusb_set_configuration');
  pointer(libusb_claim_interface) := GetProcedureAddress(VarUSBLibHandle, 'libusb_claim_interface');
  pointer(libusb_release_interface) := GetProcedureAddress(VarUSBLibHandle, 'libusb_release_interface');
  pointer(libusb_open_device_with_vid_pid) := GetProcedureAddress(VarUSBLibHandle, 'libusb_open_device_with_vid_pid');
  pointer(libusb_set_interface_alt_setting) := GetProcedureAddress(VarUSBLibHandle, 'libusb_set_interface_alt_setting');
  pointer(libusb_clear_halt) := GetProcedureAddress(VarUSBLibHandle, 'libusb_clear_halt');
  pointer(libusb_reset_device) := GetProcedureAddress(VarUSBLibHandle, 'libusb_reset_device');
  pointer(libusb_kernel_driver_active) := GetProcedureAddress(VarUSBLibHandle, 'libusb_kernel_driver_active');
  pointer(libusb_detach_kernel_driver) := GetProcedureAddress(VarUSBLibHandle, 'libusb_detach_kernel_driver');
  pointer(libusb_attach_kernel_driver) := GetProcedureAddress(VarUSBLibHandle, 'libusb_attach_kernel_driver');
  pointer(libusb_alloc_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_alloc_transfer');
  pointer(libusb_submit_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_submit_transfer');
  pointer(libusb_cancel_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_cancel_transfer');
  pointer(libusb_free_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_free_transfer');
  pointer(libusb_control_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_control_transfer');
  pointer(libusb_bulk_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_bulk_transfer');
  pointer(libusb_interrupt_transfer) := GetProcedureAddress(VarUSBLibHandle, 'libusb_interrupt_transfer');
  pointer(libusb_get_string_descriptor_ascii) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_string_descriptor_ascii');
  pointer(libusb_try_lock_events) := GetProcedureAddress(VarUSBLibHandle, 'libusb_try_lock_events');
  pointer(libusb_lock_events) := GetProcedureAddress(VarUSBLibHandle, 'libusb_lock_events');
  pointer(libusb_unlock_events) := GetProcedureAddress(VarUSBLibHandle, 'libusb_unlock_events');
  pointer(libusb_event_handling_ok) := GetProcedureAddress(VarUSBLibHandle, 'libusb_event_handling_ok');
  pointer(libusb_event_handler_active) := GetProcedureAddress(VarUSBLibHandle, 'libusb_event_handler_active');
  pointer(libusb_lock_event_waiters) := GetProcedureAddress(VarUSBLibHandle, 'libusb_lock_event_waiters');
  pointer(libusb_unlock_event_waiters) := GetProcedureAddress(VarUSBLibHandle, 'libusb_unlock_event_waiters');
  pointer(libusb_wait_for_event) := GetProcedureAddress(VarUSBLibHandle, 'libusb_wait_for_event');
  pointer(libusb_handle_events_timeout) := GetProcedureAddress(VarUSBLibHandle, 'libusb_handle_events_timeout');
  pointer(libusb_handle_events_timeout_completed) := GetProcedureAddress(VarUSBLibHandle, 'libusb_handle_events_timeout_completed');
  pointer(libusb_handle_events) := GetProcedureAddress(VarUSBLibHandle, 'libusb_handle_events');
  pointer(libusb_handle_events_completed) := GetProcedureAddress(VarUSBLibHandle, 'libusb_handle_events_completed');
  pointer(libusb_handle_events_locked) := GetProcedureAddress(VarUSBLibHandle, 'libusb_handle_events_locked');
  pointer(libusb_pollfds_handle_timeouts) := GetProcedureAddress(VarUSBLibHandle, 'libusb_pollfds_handle_timeouts');
  pointer(libusb_get_next_timeout) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_next_timeout');
  pointer(libusb_get_pollfds) := GetProcedureAddress(VarUSBLibHandle, 'libusb_get_pollfds');
  pointer(libusb_set_pollfd_notifiers) := GetProcedureAddress(VarUSBLibHandle, 'libusb_set_pollfd_notifiers');
