...
uses
     ...
     Classes,  Types, Controls, Graphics,IntfGraphics, Messages, ExtCtrls, cyTypes, cyClasses, cyGraphics;
...
type
  ...
  protected
    ...
    procedure DrawBackground(const Rect: TRect); virtual;
...
procedure TcyBaseColorMatrix.Loaded;       // !!! Not called for Dynamic controls !!!
begin
  inherited;
...
begin
  ...
  Canvas.Lock;  // Multithread drawing ...
...
procedure TcyBaseColorMatrix.DrawBackground(const Rect: TRect);
begin
  // Draw background:
...
begin
  ...
  begin
    if FCellFrameWidth > 0 then
      InflateRect(aRect, -FCellFrameWidth, -FCellFrameWidth);

    if aRect.Bottom-aRect.Top = 1 then
    begin
      if aRect.Right-aRect.Left = 1 then
      begin
        // Draw a point :
