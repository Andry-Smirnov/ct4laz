...
uses Windows, Forms, Messages, WinSpool, classes, SysUtils, ShellAPI, ShlObj, ComObj, ActiveX, cyStrUtils, cySysUtils;
...
type
  ...
  // Volta o indexo do icon do ficheiro que se ve no Windows Explorer ...
...
var
  I: Integer;
  ByteCnt : DWORD; //Nb d'octets a reserver pour recuperer le tableau de structures
  StructCnt: DWORD; //Nb de structures recuperees = nb imprimantes
  p : pointer; //pointe sur le tableau de structures retourne par EnumPrinters
...
begin
  ...
  //Le While permet d'effectuer une reservation suffisante pour le tableau de structures
  while not EnumPrinters(PRINTER_ENUM_LOCAL, nil, 2, p, ByteCnt, ByteCnt,StructCnt) Do
    if (GetLastError = ERROR_INSUFFICIENT_BUFFER)
    then p := AllocMem(ByteCnt);

  //Maintenant il ne reste plus qu'a lire le contenu des structures du tableau
  for I := 0 to StructCnt-1 do //On lit chaques structures propre a chaque imprimante
  begin
    //On se deplace dans le tableau de structures
...
//    Memo1.lines.add(#9+'Commentaire   ->   '+PrinterInfo.pComment);
  ...
  FreeMem(szCurrentPrinter, SizeOf(Char) * 256) ;                         // Liberer espace memoire ...
end;
...
begin
  try
    ...
    if szIniInfo^ <> #0 then                                                 // Si info trouvee ...
    begin
    ...
    FreeMem(szPrinterName,SizeOf(Char) * 256) ;                          // Liberer espace memoire ...
    FreeMem(szIniInfo,SizeOf(Char) * 256) ;
    FreeMem(szSection,10) ;
  except
//    on E: EOutOfMemory do ShowMessage(E.Message) ;                       // Pas d' espace memoire ...
...
function GetAvailableFilename(DesiredFileName: String): String;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            case aFileName[i] of
              ...
              '0'..'9':
                StrNumber := aFileName[i] + StrNumber;
              else
                Break;
            end;
        end;
        ...
        begin
          ...
          Result := Copy(aFileName, 1, LengthBaseName) + StrInc + StrExt;
        end
        else begin
          StrInc := '(1)';
          Result := Copy(aFileName, 1, lengthFilname - lengthStrExt) + StrInc + StrExt;
        end;
      end;

begin
  while FileExists(DesiredFileName) do
    DesiredFileName := _RenameFile(DesiredFileName);
