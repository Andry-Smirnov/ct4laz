...
uses LCLIntf, LCLType, LMessages,
      Classes,  Graphics, cyClasses, cyGraphics, cyTypes, Buttons, ComCtrls, StdCtrls;
...
(* 2016-03-10 ...
  ...
  TopLeftColor := OnCanvas.Pixels[aRect.Left, aRect.Top];
  TopRightColor := OnCanvas.Pixels[aRect.Right-1, aRect.Top];
  BottomRightColor := OnCanvas.Pixels[aRect.Right-1, aRect.Bottom-1];
  BottomLeftColor := OnCanvas.Pixels[aRect.Left, aRect.Bottom-1];

  // Draw button body if not transarent:
  if (Colors.fromColor <> clNone) or (Colors.toColor <> clNone) then
      Colors.Draw(OnCanvas, aRect);

  // Restore bounds pixel color :
  OnCanvas.Pixels[aRect.Left, aRect.Top]         := TopLeftColor;
  OnCanvas.Pixels[aRect.Right-1, aRect.Top]      := TopRightColor;
  OnCanvas.Pixels[aRect.Right-1, aRect.Bottom-1] := BottomRightColor;
  OnCanvas.Pixels[aRect.Left, aRect.Bottom-1]    := BottomLeftColor;

  // Draw frame :
  FrameColor := ColorSetPercentBrightness(Colors.toColor, -20);
  cyFrame3D(OnCanvas, aRect, FrameColor, FrameColor, 1);

  if tpActive in Flags   // Tab is active ...
  then cyFrame3D(OnCanvas, aRect, MediumColor(Colors.fromColor, Colors.ToColor), ColorSetPercentBrightness(Colors.fromColor, 40), 1)
  else DrawInnerBorders(OnCanvas, aRect, ColorSetPercentBrightness(Colors.fromColor, 40), MediumColor(Colors.fromColor, Colors.ToColor));
end;
