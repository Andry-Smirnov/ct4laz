...
type
  ...
  private
    ...
    procedure CMMouseEnter(var Message :TLMessage); message CM_MOUSEENTER;   // ct9999
    procedure CMMouseLeave(var Message :TLMessage); message CM_MOUSELEAVE;   // ct9999
...
begin
  ...
  FBevels := TcyBevels.Create(self, TcyBevel);      // ct9999
  FShadow := TcyShadowText.Create(self);            // ct9999
  FRunTimeDesign := TcyRunTimeDesign.Create(Self);  // ct9999
  ...
  FBevels.OnChange := BevelsChange;               // ct9999
  FShadow.OnChange := SubPropertiesChanged;       // ct9999
...
procedure tcyBaseFlowPanel.AlignControls(AControl: TControl; var Rect: TRect);
var
  i: Integer;
  aClientRect: TRect;
begin
  (* 2018-06-28 AControl always Nil !
  Simulate right alignment :
  if Assigned(AControl) then
    if AControl.Align = alRight then
    begin
      dx := GetClientRect.Right - Rect.Right;
      OffsetRect(Rect, dx, 0);
    end;    *)

  inherited;

  // 2018-06-28 Handle controls aligment :
  aClientRect := ClientRect;
  AdjustClientRect(aClientRect);  // Consider Bevel, border padding ...
  //=== ct9999 ===
    ...
    if Controls[i].Visible then
      if Controls[i].Align <> alNone then
        case Controls[i].Align of
          alRight:
          begin
            //Controls[i].ControlState := Controls[i].ControlState + [csAligning];
            Controls[i].SetBounds(aClientRect.Right - Controls[i].Width, Controls[i].Top, Controls[i].Width, Controls[i].Height);
            //Controls[i].ControlState := Controls[i].ControlState - [csAligning];

            // New limit for next control with same Align :
            aClientRect.Right := aClientRect.Right - Controls[i].Width;
           end;

          alBottom:
          begin
           // Controls[i].ControlState := Controls[i].ControlState + [csAligning];
            Controls[i].SetBounds(Controls[i].Left, aClientRect.Bottom - Controls[i].Height, Controls[i].Width, Controls[i].Height);
           // Controls[i].ControlState := Controls[i].ControlState - [csAligning];

            // New limit for next control with same Align :
            aClientRect.Bottom := aClientRect.Bottom - Controls[i].Height;
           end;
        end;
    ...
    if Controls[i].Visible then
      if Controls[i].Align <> alNone then
        case Controls[i].Align of
          ...
          begin
            //Controls[i].ControlState := Controls[i].ControlState + [csAligning];
            Controls[i].SetBounds(Controls[i].Left, aClientRect.Top, Controls[i].Width, Controls[i].Height);
            //Controls[i].ControlState := Controls[i].ControlState - [csAligning];

            // New limit for next control with same Align :
            aClientRect.Top := aClientRect.Top + Controls[i].Height;
           end;

          alLeft:
          begin
            //Controls[i].ControlState := Controls[i].ControlState + [csAligning];
            Controls[i].SetBounds(aClientRect.Left, Controls[i].Top, Controls[i].Width, Controls[i].Height);
           // Controls[i].ControlState := Controls[i].ControlState - [csAligning];

            // New limit for next control with same Align :
            aClientRect.Left := aClientRect.Left + Controls[i].Width;
           end;
        end;
  //===============================
end;
...
procedure tcyBaseFlowPanel.DrawBackground(const aRect: TRect);
begin
...
begin
  ...
  begin
   ...
   // Flags := DrawTextBiDiModeFlags(Flags); // ct9999
...
begin
  if FRunTimeDesign.Job <> rjNothing then
    if Assigned(FOnDoRunTimeDesign) then
      FOnDoRunTimeDesign(Self, X, Y);
