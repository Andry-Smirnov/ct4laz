...
uses LCLIntf, LCLType, LMessages,
      Classes,  Forms, Messages, Controls, Graphics, StdCtrls, ExtCtrls,
      cyTypes, cyClasses, cyPanel, cySimpleGauge;
...
type
  ...
  protected
    ...
    function ProcessMessageFromPanel(var Msg: LCLType.TMsg): Boolean; //=== ct9999 ====
  ...
  public
    ...
    procedure ProcessMessagesFromPanel(const PanelHandle: HWND = 0);
...
var
  ControlsOwner: TComponent;
begin
  if FState = psOpened then EXIT;
  FCanceled := false;
  FParent := ParentControl;
  ControlsOwner := Owner;
 // if FFlashWindow then Windows.FlashWindow(FParent.Handle, true); //=== ct9999 ====

  // Create components :
  FPanel := TcyPanel.Create(ControlsOwner);
  with FPanel do
  begin
    Visible := false;
    Parent  := FParent;
  ...
  FImage := TImage.Create(ControlsOwner);
  with FImage do
  begin
    Parent := FPanel;
    Transparent := true;
  ...
  FLabel := TLabel.Create(ControlsOwner);
  with FLabel do
  begin
    Parent := FPanel;
    Align := alClient;
  ...
  FBevel := TBevel.Create(ControlsOwner);
  with FBevel do
  begin
    Parent := FPanel;
    Align := alBottom;
  ...
  FBtnCancel := TButton.Create(ControlsOwner);
  with FBtnCancel do
  begin
    Parent := FPanel;
  ...
  FGauge := TcySimpleGauge.Create(ControlsOwner);
  with FGauge do
  begin
    Parent := FPanel;
    Smooth := true;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FImage.Picture.Graphic <> nil then
        if not (FGlyphAlign in [alTop, alBottom]) then
          inc(AutoWidth, FImage.Picture.Graphic.Width + FBorderWidth * 2);  // Glyph + separation wifth glyph ...
...
function TcyCustomProgressionPanel.ProcessMessageFromPanel(var Msg: LCLType.TMsg): Boolean;
var aHandle: HWND;
begin
  RESULT := false;
