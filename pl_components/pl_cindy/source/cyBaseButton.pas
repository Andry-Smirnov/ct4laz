...
uses
     ...
     Buttons, Dialogs, cyTypes, cyGraphics;
...
type
  ...
  private
    ...
    procedure CNDrawItem(var Message: TLMDrawItems); message LM_DRAWITEM;  // ct9999 TWMDrawItem = TLMDrawItems
    // Handle my own MouseOver variable :
    procedure CMMouseEnter(var Message: TLMessage); message CM_MOUSEENTER; // ct9999 TMessage = TLMessage
    procedure CMMouseLeave(var Message: TLMessage); message CM_MOUSELEAVE; // ct9999 TMessage = TLMessage
...
procedure TcyBaseButton.CNDrawItem(var Message: TLMDrawItems);   // ct9999
var
...
begin
  ...
  DrawItemStruct := Message.DrawItemStruct^;   // ct9999
  ...
  FCanvas.Handle := DrawItemStruct._HDC;    // ct9999
...
procedure TcyBaseButton.CMMouseEnter(var Message: TLMessage);
begin
...
procedure TcyBaseButton.CMMouseLeave(var Message: TLMessage);
begin
...
begin
  ...
  begin
    ...
    if (GetWindowLong(Handle, GWL_STYLE) and BS_MASK) <> LStyle then
      SendMessage(Handle, LM_PAINT, LStyle, 1);
  end;
end;
...
var SizeNeed, TextSize: Integer;
      ...
      begin
        ...
        CalcFlags := cyGraphics.DrawTextFormatFlags(0, taLeftJustify, tlTop, WordWrapText);    // ct9999
        CalcFlags := CalcFlags or DT_CALCRECT;
     //   CalcFlags := DrawTextBiDiModeFlags(CalcFlags);    // ct9999 For CodeTyphon

       // ct9999 For CodeTyphon
          DrawText(Canvas.Handle, PChar(Caption), -1, CalcRect, CalcFlags);
...
begin
  ...
  DrawStyle := cyGraphics.DrawTextFormatFlags(0, aAlignment, aTextLayout, WordWrap);   // ct9999
//  DrawStyle := DrawTextBiDiModeFlags(DrawStyle); // ct9999 For CodeTyphon
...
begin
  ...
  begin
    ...
    // Corners :
    Canvas.Pixels[0, 0] := cyGraphics.MediumColor(Canvas.Pixels[0, 0], LeftTopColor);
    Canvas.Pixels[0, Height-1] := cyGraphics.MediumColor(Canvas.Pixels[0, Height-1], LeftTopColor);

    Canvas.Pixels[Width-1, 0] := cyGraphics.MediumColor(Canvas.Pixels[Width-1, 0], RightBottomColor);
    Canvas.Pixels[Width-1, Height-1] := cyGraphics.MediumColor(Canvas.Pixels[Width-1, Height-1], RightBottomColor);
...
procedure TcyBaseBitBtn.CMMouseEnter(var Message: TLMessage);
begin
  if (not FMouseOver) and Enabled
  then Invalidate;
  FMouseOver := true;
...
procedure TcyBaseBitBtn.CMMouseLeave(var Message: TLMessage);
begin
  if (FMouseOver) and Enabled
  then Invalidate;
  FMouseOver := false;
...
procedure TcyBaseBitBtn.CNDrawItem(var Message: TLMDrawItems);   // ct9999
var
  DrawItemStruct: TDrawItemStruct;
  State: TButtonState;
  IsDown: Boolean;
