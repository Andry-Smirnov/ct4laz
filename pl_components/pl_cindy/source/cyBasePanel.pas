...
uses LCLIntf, LCLType, LMessages,
     Classes,  Themes, Graphics, StdCtrls, ExtCtrls,
     Controls, Messages, Dialogs, cyTypes, cyClasses, cyGraphics;
...
begin
  ...
  BevelInner := bvNone;     // ct9999 must set here
  BevelOuter := bvNone;     // ct9999 must set here
end;
...
procedure tcyBasePanel.DrawBackground(const aRect: TRect);
begin
...
begin
  ...
  //  Flags := DrawTextBiDiModeFlags(Flags);   // ct9999 for codetyphon
...
procedure tcyBasePanel.CMMouseEnter(var Msg: TLMessage);
begin
  inherited;
  MouseEnter(Self);
end;

procedure tcyBasePanel.CMMouseLeave(var Msg: TLMessage);
begin
  inherited;
  MouseLeave(Self);
end;
...
begin
  if FRunTimeDesign.Job <> rjNothing then
    if Assigned(FOnEndRunTimeDesign) then
      FOnEndRunTimeDesign(Self, X, Y);
