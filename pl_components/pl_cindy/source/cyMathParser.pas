...
type
  ...
  TProcBeforeParse = procedure (Sender: TObject; Stack: TStack) of object;
  ...
  published
    ...
    property BeforeParse: TProcBeforeParse read FBeforeParse write FBeforeParse;
  end;
...
begin
  ...
  for i:= 2 to length(Name) do
    ...
    {$ELSE}
    if not (Name[i] in ['_', 'a'..'z', '0'..'9']) then
    {$ENDIF}
      Exit;
...
begin
  ...
  //--- ct9999 ---------------
  //GetLocaleFormatSettings(LOCALE_DEFAULT, fs);
  //GetFormatSettings;
      ...
      'a'..'z', '_':  // Functions and variables must begin with a letter or with '_'
        begin
          ...
          for j := i to lengthExpr do
            {$IFDEF UNICODE}
            if CharInSet(aExpression[j], ['a'..'z', '_', '0'..'9', ' ']) then
            {$ELSE}
            if aExpression[j] in ['a'..'z', '_', '0'..'9', ' '] then
            {$ENDIF}
            begin
              // case of the function "E": (Exemple: 5E3 = 5 * 10*10*10), must be followed by a number :
              {$IFDEF UNICODE}
              if (s = 'e') and CharInSet(aExpression[j], ['0'..'9', ' ']) then
              {$ELSE}
              if (s = 'e') and (aExpression[j] in ['0'..'9', ' ']) then
              {$ENDIF}
              begin
                s := '';
        ...
        begin
          ...
          for j := i to lengthExpr do
            if aExpression[j] <> ' ' then     // 2015-02-04 Ignore spaces !
              {$IFDEF UNICODE}
              if CharInSet(aExpression[j], ['0'..'9']) then
              {$ELSE}
...
begin
  if Assigned(FBeforeParse) then
    FBeforeParse(Self, fPrefixStack);
