...
uses LCLIntf, LCLType, LMessages,
     Classes, Types, Controls, Graphics, Messages,  cyGraphics;
...
type
  ...
  private
    ...
    procedure CmMouseLeave(var Msg: TLMessage); message LM_MOUSELEAVE;
...
procedure TcySkinButton.CMButtonPressed(var Message: TLMessage);
var
  Sender: TcySkinButton;
begin
  if (csLoading in ComponentState) then exit;   //== ct9999 ===
...
procedure TcySkinButton.CmMouseLeave(var Msg: TLMessage);
begin
  if FMouseOver
  then begin
    FMouseOver := false;
