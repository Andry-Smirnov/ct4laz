...
uses Classes, Themes, Types, Graphics, Controls, Buttons,  {$IFDEF DELPHI2009_OR_ABOVE} pngimage, {$ENDIF}
      cyTypes, cyClasses, cyGraphics, cyBaseSpeedButton;
...
procedure TcyAdvSpeedButton.SetImageList(const Value: TImageList);
begin
  FImageList := Value;
  Invalidate;
...
begin
  ...
  if Flat
  then begin
    if PaintButton = FButtonHot     // Show hot state on flat buttons :
    then begin
      // if ThemeControl(Self) and (FlatHotStyle = hsWindowsTheme)
      ...
      then begin
        ...
        if FlatHotStyle <> hsHidden
        then PaintBorders.DrawBevels(Canvas, Rect, true);
    end;

    if PaintButton = FButtonDown    // Show down state on flat buttons :
    then begin
