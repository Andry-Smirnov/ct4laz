...
uses
     ...
     Classes, Themes, Forms, StdCtrls, Messages, Graphics, Controls,
     Buttons, Dialogs, cyTypes, cyClasses, cyGraphics;
...
begin
  ...
  self.SetInitialBounds(0,0,90,20); //=== ct9999 ====
...
begin
 ...
 // with Params do
 //   Style := Style or SS_NOTIFY or MF_OWNERDRAW;
end;
...
procedure TcyBaseStaticText.CMEnabledChanged(var Message: TLMessage);
begin
  Inherited;
  Invalidate;
end;

procedure TcyBaseStaticText.CMMouseEnter(var Message: TLMessage);
begin
  FMouseOver := true;
  Inherited;
end;

procedure TcyBaseStaticText.CMMouseLeave(var Message: TLMessage);
begin
  FMouseOver := false;
  Inherited;
end;
...
//=== ct9999 ==============
procedure TcyBaseStaticText.WMPaint(var Msg: TLMPaint);
var
  State: TButtonState;
begin
...
begin
  ...
  DrawStyle := cyGraphics.DrawTextFormatFlags(0, FCaptionAlignment, FCaptionLayout, FWordWrap, FCaptionRender);
  if not FShowAccelChar then DrawStyle := DrawStyle or DT_NOPREFIX;
 // DrawStyle := DrawTextBiDiModeFlags(DrawStyle);

  if FCaptionOrientation = coHorizontal then
  begin
