...
uses LCLIntf, LCLType, LMessages, SysUtils,
     Classes,  Types, Graphics, Controls, cyGraphics;
...
begin
 ...
 // ct9999
 // FAttractFrameDC := GetDCEx(FDrawMotionControl.Handle, 0, DCX_CACHE or DCX_CLIPSIBLINGS or DCX_LOCKWINDOWUPDATE);
  FAttractFrameDC := GetDC(FDrawMotionControl.Handle);

  if FBrush = nil then
  begin
    FBrush := TBrush.Create;
{$IF NOT DEFINED(LINUX)}
   // FBrush.Bitmap := AllocPatternBitmap(clBlack, clWhite); // ct9999
    FBrush.Color := clGray;
{$ELSE}
    FBrush.Color := clGray;
{$IFEND}
