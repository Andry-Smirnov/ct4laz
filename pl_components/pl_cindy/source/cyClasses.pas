...
uses
     ...
     Messages,//types,
     Classes, Forms, Buttons, Graphics, Math, Controls, ExtCtrls, StdCtrls,SysUtils, cyTypes, cyGraphics;
...
type
  ...
  public
    ...
    function DetermineJobAtPos(const X, Y: Integer): TRunTimeDesignJob;
    procedure StartJob(const X, Y: Integer);
    procedure DoJob(const X, Y: Integer);
    procedure EndJob(const X, Y: Integer);
  ...
  published
    ...
    property ParentControl: Boolean read FParentControl write FParentControl default false;
  end;
  ...
  procedure cyDrawBgPicture(aCanvas: TCanvas; const aRect: TRect; aBgPicture: TcyBgPicture);
...
begin
  ...
  begin
    ...
    FParentControl := TcyRunTimeDesign(Source).FParentControl;
  end;
...
function TcyRunTimeDesign.DetermineJobAtPos(const X, Y: Integer): TRunTimeDesignJob;
var
...
procedure TcyRunTimeDesign.StartJob(const X, Y: Integer);
begin
  if not Assigned(FControl) then Exit;
  ...
  if Job <> rjNothing then  // Are we doing something?
  begin
    FFromX := X;
    FFromY := Y;

    if ParentControl and Assigned(FControl.Parent) then
    begin
      FHandledControl := FControl.Parent;
...
//        ClientToScreenCoord(FControl, FFromX, FFromY);
//        ScreenToClientCoord(FHandledControl.Parent, FFromX, FFromY);

//          FFromX := FFromX;
//          FFromY := FFromY;

//        ClientToScreenCoord(FControl, X, Y);
//        ScreenToClientCoord(FHandledControl, X, Y);
      end;        *)
    end
    else
      FHandledControl := FControl;

    while Assigned(FHandledControl) do
    begin
      FBoundsRect := FHandledControl.BoundsRect;

      if FHandledControl.Align = alClient then
      begin
        if Assigned(FHandledControl.Parent) then
        begin
          FHandledControl := FHandledControl.Parent;

//          FFromX := FFromX + FControl.Left;
//          FFromY := FFromY + FControl.Top;
        end
        else
          Break;
      end
...
procedure TcyRunTimeDesign.DoJob(const X, Y: Integer);
      ...
      begin
        ...
        then
          ...
          then begin
            ...
            then Limit := Screen.Width - FHandledControl.Width
            else Limit := FHandledControl.Parent.Width - FHandledControl.Width;
      ...
      begin
        ...
        then
          ...
          then begin
            if FHandledControl.Parent = Nil
            then Limit := Screen.Height - FHandledControl.Height
            else Limit := FHandledControl.Parent.Height - FHandledControl.Height;
      ...
      begin
        ...
        then begin
          ...
          else Limit := FHandledControl.Parent.Width;
      ...
      begin
        ...
        then begin
          ...
          else Limit := FHandledControl.Parent.Height;
...
begin
  if FJob = rjNothing then
  begin
    JobAtPos := DetermineJobAtPos(X, Y);
    Exit;
  end;

  if not Assigned(FHandledControl) then Exit;

  if FJob = rjMove
  then begin
    FHandledControl.Left := CorrectMoveLeft(FHandledControl.Left + X - FFromX);
    FHandledControl.Top := CorrectMoveTop(FHandledControl.Top + Y - FFromY);
  end
  else begin
    if FJob in [rjResizeRight, rjResizeTopRight, rjResizeBottomRight]
    then begin
      FBoundsRect.Right := CorrectResizeRight(FBoundsRect.Right + X - FFromX);
      FFromX := FBoundsRect.Right - FBoundsRect.Left;
    end
    else
      if FJob in [rjResizeLeft, rjResizeTopLeft, rjResizeBottomLeft]
      then begin
        FBoundsRect.Left := CorrectResizeUpperLeft(FBoundsRect.Left - FFromX + X);

        // Check constraints values to avoid moving the component:
        if FHandledControl.Constraints.MaxWidth > 0
        then
          if FBoundsRect.Right - FBoundsRect.Left > FHandledControl.Constraints.MaxWidth
          then FBoundsRect.Left := FBoundsRect.Right - FHandledControl.Constraints.MaxWidth;

        if FHandledControl.Constraints.MinWidth > 0
        then
          if FBoundsRect.Right - FBoundsRect.Left < FHandledControl.Constraints.MinWidth
          then FBoundsRect.Left := FBoundsRect.Right - FHandledControl.Constraints.MinWidth;
      end;

    if FJob in [rjResizeBottom, rjResizeBottomLeft, rjResizeBottomRight]
    then begin
      FBoundsRect.Bottom := CorrectResizeBottom(FBoundsRect.Bottom + Y - FFromY);
      FFromY := FBoundsRect.Bottom - FBoundsRect.Top;
    end
    else
      if FJob in [rjResizeTop, rjResizeTopLeft, rjResizeTopRight]
      then begin
        FBoundsRect.Top := CorrectResizeUpperLeft(FBoundsRect.Top - FFromY + Y);

        // Check constraints values to avoid moving the component:
        if FHandledControl.Constraints.MaxHeight > 0
        then
          if FBoundsRect.Bottom - FBoundsRect.Top > FHandledControl.Constraints.MaxHeight
          then FBoundsRect.Top := FBoundsRect.Bottom - FHandledControl.Constraints.MaxHeight;

        if FHandledControl.Constraints.MinHeight > 0
        then
          if FBoundsRect.Bottom - FBoundsRect.Top < FHandledControl.Constraints.MinHeight
          then FBoundsRect.Top := FBoundsRect.Bottom - FHandledControl.Constraints.MinHeight;
      end;

    FHandledControl.SetBounds(FBoundsRect.Left, FBoundsRect.Top, FBoundsRect.Right - FBoundsRect.Left, FBoundsRect.Bottom - FBoundsRect.Top);
  end;
end;

procedure TcyRunTimeDesign.EndJob(const X, Y: Integer);
begin
  // 2016-04-20 ... if not Assigned(FHandledControl) then Exit;
