...
uses
   ...
   Classes, Buttons, StdCtrls, Controls, ComCtrls, Graphics, Messages,
   cyBaseTabControl,cyClasses, cyGraphics;
...
type
  ...
  public
    ...
    property Canvas2: TCanvas read FCanvas2; //=== ct9999 ======
  published
...
begin
  ...
  FCanvas2 := TControlCanvas.Create;
  TControlCanvas(FCanvas2).Control := Self;
    ...
    with PRect(Msg.LParam)^ do
      if ThinBorder then
        case TabPosition of
          tpTop:
          begin
            if Style <> tsTab then     //  if MultiLine and (Style <> tsTab)
            begin
              Left := 0;
              Right := Width;
              Bottom := Height;
      ...
      if ThinBorder then
        case TabPosition of
          ...
          begin
            ...
            Left := 0;              // Same as dec(Left, 4);
            Right := Self.Width;    // Same as inc(Right, 4);
            Dec(Top, 3);
            Bottom := Self.Height;  // Same as inc(Bottom, 4);
          end;
          ...
          begin
            // Only tabs !!
            dec(Left, 2);
            Right := Self.Width;        // Same as inc(Right, 2);
            Top := 0;                   // Same as dec(Top, 2);
            Bottom := Self.Height;      // Same as inc(Bottom, 2);
          end;

          tpRight:
          begin
            // Only tabs !!
            Left := 0;                  // Same as dec(Left, 2);
            inc(Right, 2);
            Top := 0;                   // Same as dec(Top, 2);
            Bottom := Self.Height;      // Same as inc(Bottom, 2);
          end;
...
begin
  ...
  begin
    ...
    if Draw then
      DrawTabStyleCaption(Canvas2, aCaption, TabPosition, aRect);
  end
  else
    DrawTabStyleCaption(Canvas2, aCaption, TabPosition, aRect);
...
begin
  ...
  if Tab = TabIndex
  then DoDrawTabStyleBackground(Canvas2, Tab, FStyle, TabPosition, aRect, FActiveTabColors, Parameters)
  else DoDrawTabStyleBackground(Canvas2, Tab, FStyle, TabPosition, aRect, FInactiveTabColors, Parameters);
  ...
  begin
    ...
    Images.Draw(Canvas2, aGraphicX, aGraphicY, Pages[aPage].ImageIndex);
  end
  else
    TextRect := aRect;

  if Tab = TabIndex
  then DoDrawTabStyleCaption(Canvas2, Tab, Pages[aPage].Caption, TextRect, FActiveTabFont)
  else DoDrawTabStyleCaption(Canvas2, Tab, Pages[aPage].Caption, TextRect, FInactiveTabFont);
...
var SizeNeed, TextSize: Integer;
      ...
      begin
        ...
        CalcFlags := cyGraphics.DrawTextFormatFlags(0, taLeftJustify, tlTop, WordWrapText);
        CalcFlags := CalcFlags or DT_CALCRECT;
       // CalcFlags := DrawTextBiDiModeFlags(CalcFlags);

        {$IFDEF UNICODE}
          cyDrawText(Canvas2.Handle, Caption,CalcRect, CalcFlags);
        {$ELSE}
          cyDrawText(Canvas2.Handle, PChar(Caption), CalcRect, CalcFlags);
        {$ENDIF}
