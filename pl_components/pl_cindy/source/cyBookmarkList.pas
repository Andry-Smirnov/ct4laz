...
begin
  ...
  RESULT := Pointer(DataSource.Dataset.Bookmark);
end;
...
begin
  ...
  if (aBookmark <> nil) and (Find(aBookmark, Index) = false)
  then begin
    RESULT := true;
...
begin
  ...
  if (aBookmark <> nil) and (Find(aBookmark, Index) = true)
  then begin
    RESULT := true;
    DeleteItem(Index);
...
begin
  ...
  if (Current = nil) or (Find(Current, Index) = Value) then Exit;
