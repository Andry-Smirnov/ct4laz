...
uses LCLIntf, LCLType, LMessages,
     Classes,Graphics, Controls, StdCtrls, SysUtils, cyGraphics, cyTypes, cyEdit, cyMathParser;
...
type
  ...
  public
    ...
    function ValidateValue(aText: String): TEditValidateResult; override;
  ...
  published
    ...
    property CanParse: Boolean read FCanParse write SetCanParse default false;
...
begin
  ...
  fs:=DefaultFormatSettings;  //=== ct9999 ====
...
procedure TcyEditFloat.SetCanParse(const Value: Boolean);
begin
  FCanParse := Value;
...
function TcyEditFloat.ValidateValue(aText: String): TEditValidateResult;
var
  Rslt: Extended;
begin
  Result := evValid;
...
procedure TcyEditFloat.WMPaint(var Msg: TLMPaint); //=== ct9999 ====
begin
...
begin
  ...
  begin
    ...
    {$IFDEF UNICODE}
    DrawStyle := DT_WORDBREAK or DT_TOP or Alignments[Alignment] ;//or DrawTextBiDiModeFlagsReadingOnly;     //=== ct9999 ===
    {$ELSE}
    DrawStyle := DT_WORDBREAK or DT_TOP or Alignments[taLeftJustify]; //or DrawTextBiDiModeFlagsReadingOnly; //=== ct9999 ===
    {$ENDIF}
    cyGraphics.cyDrawText(FCanvas.Handle, DisplayText, R, DrawStyle);
  end;
...
var
  ...
  DefaultValue: Double;
  PosDec, CurrentDecimals, i: Integer;
begin
  ValidateText;
  ...
  begin
    ...
    begin
      ...
      CurrentDecimals := Length(aText) - PosDec;

      while CurrentDecimals > FPrecision do
      begin
        Delete(aText, Length(aText), 1);
        Dec(CurrentDecimals);
      end;

      for i := CurrentDecimals + 1 to FPrecision do
        aText := aText + '0';
...
// 2018-10-04 :
procedure TcyEditFloat.ValidateText;
var
  Parser: TcyMathParser;
begin
  if FCanParse then
    ...
    begin
      ...
      if Parser.Parse
      then Value := Parser.ParserResult
      else HandleInvalidValue;
