...
uses
     ...
     Classes,  Forms, Graphics, Math, Buttons, Controls, ExtCtrls, {$IFDEF DELPHI2009_OR_ABOVE} pngimage, {$ENDIF}
     StdCtrls, SysUtils, cyTypes;
...
procedure DrawInnerBorders(Canvas: TCanvas; var Rect: TRect; const TopColor, BottomColor: TColor);
...
// Text functions :
procedure cyDrawSingleLineText(Canvas: TCanvas; const Text: String; const ARect: TRect; const Alignment: TAlignment; const TextLayout: TTextLayout; const IndentX: Integer = 0; const IndentY: Integer = 0);
...
procedure cyDrawText(const CanvasHandle: Cardinal; const Text: String; var Rect: TRect; TextFormat: LongInt);
...
function PngImageIsTransparentAtPos(aPngImage: TPortableNetworkGraphic; aPoint: TPoint): boolean;

// Draw a canvas Rect into another (Windows GDI library - not transparent) :
procedure DrawCanvas(Destination: TCanvas; const DestRect: TRect; Source: TCanvas; SourceRect: TRect); overload;
procedure DrawCanvas(Destination: TCanvas; const DestRect: TRect; Src: TCanvas; SrcRect: TRect; TransparentColor: TColor;
...
procedure DrawGraphic(Destination: TCanvas; const DestRect: TRect; aGraphic: TGraphic; SrcRect: TRect; TransparentColor: TColor;
           const aStyle: TBgStyle = bgNormal; const aPosition: TBgPosition = bgTopLeft;
           const IndentX: Integer = 0; const IndentY: Integer = 0;
           const IntervalX: Integer = 0; const IntervalY: Integer = 0;
           const RepeatX: Integer = 1; const RepeatY: Integer = 1); overload;
...
// Other functions:
function ClientToScreenRect(aControl: TControl; const aControlRect: TRect): TRect;
function ScreenToClientRect(aControl: TControl; const aScreenRect: TRect): TRect;
procedure ClientToScreenCoord(aControl: TControl; var InOutX, InOutY: Integer);
procedure ScreenToClientCoord(aControl: TControl; var InOutX, InOutY: Integer);
...
Procedure RotateBitmap_ads(SourceBitmap : TBitmap; out DestBitmap : TBitmap; Center : TPoint; Angle : Double);  //--- ct9999 ----------

function CopyPalette(Source: hPalette): hPalette; // fix by Avra
...
begin
  ...
  else DrawInnerBorders(Canvas, Rect, BorderLeftTopColor, BorderRightBottomColor);
...
begin
  ...
  then begin
    ...
    //--- ct9999 -----------------------------------
        ...
        if aPicture.Graphic is TPortableNetworkGraphic
        then
          RESULT := PngImageIsTransparentAtPos(TPortableNetworkGraphic(aPicture.Graphic), aPoint);
   //----------------------------------------------
end;
...
function PngImageIsTransparentAtPos(aPngImage: TPortableNetworkGraphic; aPoint: TPoint): boolean;
begin
  RESULT := aPngImage.Canvas.Pixels[aPoint.X, aPoint.Y] = aPngImage.TransparentColor;
end;
//-----------------------------------------------------------------

procedure ClientToScreenCoord(aControl: TControl; var InOutX, InOutY: Integer);
var
  aControlPt, aScreenPt: TPoint;
begin
  aControlPt := Point(InOutX, InOutY);
  aScreenPt := aControl.ClientToScreen(aControlPt);
  InOutX := aScreenPt.X;
  InOutY := aScreenPt.Y;
end;
...
var
  aControlPt, aScreenPt: TPoint;
begin
  aScreenPt := Point(InOutX, InOutY);
  aControlPt := aControl.ScreenToClient(aScreenPt);
  InOutX := aControlPt.X;
  InOutY := aControlPt.Y;
end;
...
begin
  ...
  if r > 127 then r := 255 - r else r := r + 127;
  if g > 127 then g := 255 - g else g := g + 127;
  if b > 127 then b := 255 - b else b := b + 127;
...
procedure DrawCanvas(Destination: TCanvas; const DestRect: TRect; Source: TCanvas; SourceRect: TRect);
begin
...
var
  ...
  CalcRect: TRect;
begin
  if NeedRectAdjustment then    // We have to adjust aRect if DT_VCENTER / DT_BOTTOM and DT_WORDBREAK
  begin
    CalcRect := Rect;
    CalcFlags := TextFormat or DT_CALCRECT;


    DrawText(CanvasHandle, PChar(Text), -1, CalcRect, CalcFlags);    //--- ct9999 ----------
    ...
    DrawText(CanvasHandle, PChar(Text), -1, Rect, TextFormat);     //--- ct9999 ----------
...
begin
  ...
  Canvas.Pen.Width := 1;
  Canvas.Pen.Mode := pmMask;  // pmNotXor
  Canvas.Pen.Color := cyGraphics.ColorModify(PageColor, -40, -40, -40); // ct9999
