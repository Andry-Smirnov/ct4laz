...
{$ENDIF}
  ...
  TMaskOptions = (moNone, moOptional, moOptionalDeleteIt, moDeleteIt);
  TMaskPartType = (mtUndefined, mtMaskPartStringValue, mtMaskPartCustomChar, mtAnyChar, mtAlphaNumChar, mtNumber, mtAlphaChar, mtUppLetter, mtLowLetter, mtOtherChar, mtDeleteChar);
...
{$ENDIF}
  ...
  MaskRulesMsg =
   ...
   '%       - If is the first char on custom chars set, it deactivate default'            + #13#10 +
   '%%      - Prefix for optional char, quoted string or char in set for deletion'        + #13#10 +
   'R       - Prefix any char or quoted string for deletion'                              + #13#10 +
   '''Hello'' - string ''Hello'' (quoted) expected'                                       + #13#10 +
   '[abc]   - Custom chars set, a, b or c expected. First char will be used as default'   + #13#10 +
...
// *** SubString functions *** //
  ...
  procedure SubString_Add(var Str: String; const Separator: Char; const Value: String; const AllowEmpty: Boolean = True);
...
// *** String functions *** //
  ...
  procedure String_AddLine(var Str: String; const LineValue: String);
  ...
  function TryString_ToDate(Str: String; var RsltValue: TDateTime): Boolean;
  ...
  function String_AtPos(SubStr: String; Str: String; atPos: Integer; const CaseSensitive: TCaseSensitive): Boolean;
  // English: Retrieve true if subString at predefined position
  // Français: Renvoie vrai si sous chaine à la position spécifiée
  ...
  function String_Copy(const Str: String; Between1: String; const Between1MustExist: Boolean; Between2: String; const Between2MustExist: Boolean; const CaseSensitive: TCaseSensitive = csCaseSensitive): String; overload;
...
(*
 ...
 %%      - Remove next char. If on custom chars set, it remove char
 0       - Number expected
...
function GetNextMaskPart(var InOutMask, RsltMaskPartParamStr: string; var RsltMaskPartType: TMaskPartType; var RsltOptions: TMaskOptions): Boolean;
function IsValidMask(const aMask: String): Boolean;
function MaskPartCount(aMask: string; var RsltOptionalCount, RsltDeleteCount: Integer): Integer;
function MaskCharsCount(aMask: string; var RsltOptionalCount, RsltDeleteCount: Integer): Integer;
...
function GetMask(const aString: string; StringResult: string): string;
...
begin
  ...
  if i = 0 then         // Substring does not exists ...
  begin
    SubStrCount := SubString_Count(Str, Separator);
...
begin
  ...
  if DecSepar <> ''
  then fs.DecimalSeparator := DecSepar[1]
  else fs.DecimalSeparator := SysUtils.FormatSettings.DecimalSeparator; // 2018-12-07 We need to put something else fs.DecimalSeparator = #0 and converting '1' will return 1.5 !!!
...
function String_AtPos(SubStr: String; Str: String; atPos: Integer; const CaseSensitive: TCaseSensitive): Boolean;
begin
  if atPos < 1 then atPos := 1;   // From first char ...
  ...
  Result := Copy(Str, atPos, Length(SubStr)) = SubStr;
end;
...
function String_Copy(const Str: String; Between1: String; const Between1MustExist: Boolean; Between2: String; const Between2MustExist: Boolean; const CaseSensitive: TCaseSensitive = csCaseSensitive): String;
var
...
var
  i: Integer;
  WasSpecialChar, FoundPrefix, FoundDomain, FoundCountry: Boolean;
...
begin
  ...
  Result := FoundPrefix and FoundDomain and FoundCountry;
end;
      ...
      if (aMask[i] = MaskBeginCustomChars) and (not MaskPartCustomCharsValueStarted) then
        ...
        if (aMask[i] = MaskEndCustomChars) and (MaskPartCustomCharsValueStarted) then
          ...
          if (not MaskPartStringValueStarted) and (not MaskPartCustomCharsValueStarted) then
            {$IFDEF UNICODE}
            if not CharInSet(aMask[i], [cQuoteChar, MaskPrefixOptional, MaskPrefixRemove, MaskAnyChar, MaskAlphaNumChar, MaskNumber, MaskOptionalNumber, MaskAlphaChar, MaskUppercaseChar, MaskLowercaseChar, MaskOtherChar]) then
            {$ELSE}
            if not (aMask[i] in [cQuoteChar, MaskPrefixOptional, MaskPrefixRemove, MaskAnyChar, MaskAlphaNumChar, MaskNumber, MaskOptionalNumber, MaskAlphaChar, MaskUppercaseChar, MaskLowercaseChar, MaskOtherChar]) then
            {$ENDIF}
              Result := false;
...
function GetNextMaskPart(var InOutMask, RsltMaskPartParamStr: string; var RsltMaskPartType: TMaskPartType; var RsltOptions: TMaskOptions): Boolean; // v2.1 ...
begin
  Result := false;
  if InOutMask = '' then Exit;

  if InOutMask[1] = MaskPrefixRemove then
  begin
    RsltOptions := moDeleteIt;
    ...
    begin
      RsltOptions := moOptional;
      Delete(InOutMask, 1, 1);
      if InOutMask = '' then Exit;

      // 2018-04-27 Handle deletion :
      if InOutMask[1] = MaskPrefixOptional then
      begin
        RsltOptions := moOptionalDeleteIt;
        Delete(InOutMask, 1, 1);
        if InOutMask = '' then Exit;
      end;
    end
  ...
  case InOutMask[1] of
    ...
    MaskPrefixRemove:     RsltMaskPartType := mtUndefined;
  ...
  if RsltMaskPartType = mtUndefined then
    ...
    begin
      ...
      begin
        ...
        if pos('%', RsltMaskPartParamStr) <> 0 then
          RsltOptions := moOptional;
        Result := RsltMaskPartParamStr <> '';
      end
      else begin
...
function MaskPartCount(aMask: string; var RsltOptionalCount, RsltDeleteCount: Integer): Integer;     // v2.1 ...
var
  Mask, MaskPartParamStr: string;
  MaskPartType: TMaskPartType;
  RsltOptions: TMaskOptions;
begin
  Result := 0;
  RsltOptionalCount := 0;
  RsltDeleteCount := 0;
  Mask := aMask;
  while GetNextMaskPart(Mask, MaskPartParamStr, MaskPartType, RsltOptions) do
  begin
    Inc(Result);

    if RsltOptions in [moOptional, moOptionalDeleteIt] then  // 2018-10-10 ... if RsltOptions = moOptional then
      // 2018-10-09
      if MaskPartType = mtMaskPartStringValue
      then RsltOptionalCount := RsltOptionalCount + Length(MaskPartParamStr)   // 2018-10-04
      else Inc(RsltOptionalCount);
      // Inc(RsltOptionalCount);

    if RsltOptions in [moOptionalDeleteIt, moDeleteIt] then
      // 2018-10-09
      if MaskPartType = mtMaskPartStringValue
      then RsltDeleteCount := RsltDeleteCount + Length(MaskPartParamStr)   // 2018-10-04
      else Inc(RsltDeleteCount);
      // Inc(RsltDeleteCount);
  end;
end;
...
begin
  ...
  begin
    ...
    if RsltOptions in [moOptional, moOptionalDeleteIt] then
      // 2018-10-09
      if MaskPartType = mtMaskPartStringValue
      then RsltOptionalCount := RsltOptionalCount + Length(MaskPartParamStr)   // 2018-10-04
      else Inc(RsltOptionalCount);
...
begin
  ...
  begin
    ...
    begin
      ...
      GetNextMaskPart(InOutMask, RsltMaskPartParamStr, RsltMaskPartType, RsltOptions);  // Remove single mask part from left and try again ...
  end;
end;
  ...
  OptionalCount: Integer;
      ...
      begin
        ...
        if (OptionalType = MaskPartType) or (MaskPartTypeCompatible(MaskPartType, OptionalType)) then
          ...
          begin
            if MaskPartType = mtMaskPartStringValue
            then OptionalCount := OptionalCount - Length(MaskPartParamStr)   // 2018-10-04
            else Dec(OptionalCount);
      ...
      begin
        ...
        while Result and GetNextMaskPart(aMask, MaskPartParamStr, MaskPartType, RsltOptions) do
        begin
...
begin
  ...
  while GetNextMaskPart(aMask, MaskPartParamStr, MaskPartType, RsltOptions) do
  begin
    if (OptionalType <> MaskPartType) and (not MaskPartTypeCompatible(MaskPartType, OptionalType)) then
    begin
      OptionalType := MaskPartType;
      ...
      case MaskPartType of
        ...
        begin
          ...
          if Result then
            ...
            if DecOptionalCount // if RsltOptions
            then Result := true
            else aMask := MaskPartParamStr + aMask;
        end;
...
function MergeMaskToMatchedString(var InOutMask: string; aString: string): String;       // v2.1 ...
var
  MaskPartParamStr: string;
  MaskPartType: TMaskPartType;
  RsltOptions: TMaskOptions;
...
begin
  ...
  while GetNextMaskPart(InOutMask, MaskPartParamStr, MaskPartType, RsltOptions) do
  begin
    if (OptionalType <> MaskPartType) and (not MaskPartTypeCompatible(MaskPartType, OptionalType)) then
    begin
    ...
    if RsltOptions in [moOptional, moOptionalDeleteIt] then // 2018-10-04 ... if RsltOptions = moOptional then
      if MaskPartType = mtMaskPartStringValue
      then OptionalCount := OptionalCount + Length(MaskPartParamStr)  // 2018-10-04
      else Inc(OptionalCount);
    ...
    begin
      ...
      case MaskPartType of
        ...
        begin
          ...
          begin
            if not (RsltOptions in [moOptionalDeleteIt, moDeleteIt]) then   // 2018-10-09
...
function MergeMaskToMatchedString2(const aMask: string; const aString: string): String;    // v2.1 ...
var
...
begin
  ...
  begin
    MaskPartCarType := Char_GetType(aString[i]);

    case MaskPartCarType of
      ctNumeric:             MaskPart := '0';
      ctAlphabeticLowercase: MaskPart := 'L';
      ctAlphabeticUppercase: MaskPart := 'U';
      else                   MaskPart := QuotedStr(aString[i]);
    end;

    if StringResult = '' then
      MaskPart := 'R' + MaskPart
    else
      if Char_GetType(StringResult[1]) <> MaskPartCarType
      then MaskPart := 'R' + MaskPart
      else Delete(StringResult, 1, 1);

    Result := Result + MaskPart;
  end;
end;

function TryString_ToDate(Str: String; var RsltValue: TDateTime): Boolean;
begin
  String_SubstFast('/', FormatSettings.DateSeparator, Str);
  String_SubstFast('-', FormatSettings.DateSeparator, Str);
  String_SubstFast('.', FormatSettings.DateSeparator, Str);

  Result := TryStrToDateTime(Str, RsltValue);
end;
