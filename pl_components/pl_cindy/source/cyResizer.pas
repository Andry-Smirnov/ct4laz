...
uses LCLIntf, LCLType, LMessages,
     Classes,Forms, RTLConsts, Controls, Graphics, Messages, cyTypes, cyGraphics;
...
type
  ...
  TcyBaseWinControl = class(TWinControl)
  protected
  public
  end;
  ...
  private
    ...
    procedure WMMove(var Message: TLMessage); message LM_MOVE;  // In order to know when TcyResizer was moved/scrolled by a TScrollBox for exemple
...
begin
  ...
  begin
    Canvas.Lock;      // 2017-03-30 Multithread drawing: When using PaintTo to draw on a canvas, you must lock the canvas first
    TWinControl(aControl).PaintTo(Canvas, aControl.Left - Left, aControl.Top - Top);
    Canvas.UnLock;    // 2017-03-30
  end
  else
...
var
  ...
  FWinControl: TcyBaseWinControl;
  EdgeSize: Integer;
begin
  Result := classes.Rect(0, 0, Item.FControl.Width-1, Item.FControl.Height-1);

  // 2017-03-30 Correct Rect for TWinControl regarding BorderWidth/Bevel :
  if Item.FControl is TWinControl then
  begin
    FWinControl := TcyBaseWinControl(Item.FControl);

    if Integer(FWinControl.BorderWidth) <> 0 then
      OffsetRect(Result, -Integer(FWinControl.BorderWidth), -Integer(FWinControl.BorderWidth));
   {----------- ct9999 -----------------------------
    if FWinControl.BevelKind <> bkNone then
    begin
      EdgeSize := 0;
      if FWinControl.BevelInner <> bvNone then Inc(EdgeSize, FWinControl.BevelWidth);
      if FWinControl.BevelOuter <> bvNone then Inc(EdgeSize, FWinControl.BevelWidth);

      if beLeft in BevelEdges then
      begin
        Dec(Result.Left, EdgeSize);
        Dec(Result.Right, EdgeSize);
      end;

      if beTop in BevelEdges then
      begin
        Dec(Result.Top, EdgeSize);
        Dec(Result.Bottom, EdgeSize);
      end;

      // beRight and beBottom don' t affect position ...
      // if beRight in BevelEdges then Dec(Result.Right, EdgeSize);
      // if beBottom in BevelEdges then Dec(Result.Bottom, EdgeSize);
    end;   }
  end;
...
procedure TcyResizer.WMMove(var Message: TLMessage);
begin
  Inherited;
